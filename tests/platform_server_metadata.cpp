...
int main()
{
	...
	assert(Metadata.m_Official && !Metadata.m_Modded && Metadata.m_AuthPolicy == 2 &&
		   strcmp(Metadata.m_aModHash, "none") == 0);
	const char *pHash = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
	char aTags[160];
	snprintf(aTags, sizeof(aTags), "modhash=%s,auth=1,official=0,modded=1", pHash);
	assert(PlatformServerMetadataParse(aTags, &Metadata) && !Metadata.m_Official && Metadata.m_Modded &&
		   Metadata.m_AuthPolicy == 1);
