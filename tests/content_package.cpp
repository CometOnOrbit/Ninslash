...
int main()
{
	...
	str_format(aManifestPath,sizeof(aManifestPath),"%s/ninslash_content.json",aRoot); str_format(aExtraPath,sizeof(aExtraPath),"%s/extra.txt",aRoot);
	WriteFile(aMapPath,"map"); WriteFile(aScriptPath,"function on_event() end");
	const char *pTemplate="{\"schema_version\":1,\"content_type\":\"mod\",\"published_file_id\":\"42\",\"name\":\"Safe\",\"description\":\"Test content\",\"version\":\"1\",\"author\":\"A\",\"target_protocol\":\"test\",\"content_hash\":\"%s\",\"content_rating\":\"everyone\",\"api_version\":1,\"capabilities\":[\"resources\"],\"maps\":[\"maps/a.map\"],\"scripts\":[\"rules/main.lua\"]}";
	...
	CContentManifest Manifest; assert(ContentManifestParse(aJson,(int)strlen(aJson),"test",&Manifest,aError,sizeof(aError)));
	char aHash[65]; assert(ContentPackageComputeHash(aRoot,Manifest,aHash,aError,sizeof(aError)));
	char aChangedJson[2048], aChangedHash[65]; str_copy(aChangedJson,aJson,sizeof(aChangedJson)); char *pDescription=strstr(aChangedJson,"Test content"); assert(pDescription); pDescription[0]='B'; CContentManifest Changed; assert(ContentManifestParse(aChangedJson,(int)strlen(aChangedJson),"test",&Changed,aError,sizeof(aError))); assert(ContentPackageComputeHash(aRoot,Changed,aChangedHash,aError,sizeof(aError))); assert(str_comp(aHash,aChangedHash)!=0);
	...
	assert(ContentPackageStage(aRoot,aWorkshop,"42","test",&Manifest,aStaged,sizeof(aStaged),aError,sizeof(aError)));
