...
要在专用服务器上启用 Mod，请把各个包目录安装到服务器保存目录下的 `workshop/` 文件夹，然后只配置根包 ID，例如：

```cfg
sv_mod_ids 9000000001
```

包文件夹可以使用易读名称；内容身份由 `ninslash_content.json` 中的 `published_file_id` 决定。服务端启动时会解析依赖，并根据实际安装内容自动计算 `sv_mod_hash`，无需从客户端手工复制哈希。客户端必须启用相同版本的包；不一致时服务端日志会同时列出两端的集合哈希和 ID。
