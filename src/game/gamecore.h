...
inline vec2 CalcTPos(vec2 Pos, vec2 Velocity, vec2 Velocity2, float Curvature, float Speed, float Time)
{
	vec2 n;
	n.x = Pos.x + sin(Velocity.x) * (Velocity.x > 0.0f ? 1 : -1) * Velocity2.x * Time +
		  sin(Time * 0.03f) * min(Time * 0.1f, 48.0f) * Velocity.y;
	n.y = Pos.y + sin(Velocity.y) * (Velocity.y > 0.0f ? 1 : -1) * Velocity2.y * Time +
		  sin(Time * 0.04f) * min(Time * 0.1f, 48.0f) * Velocity.y;
	Time *= Speed;
	n.x += Velocity.x * Time;
...
template <typename T> inline T SaturatedAdd(T Min, T Max, T Current, T Modifier)
{
	if(Modifier < 0)
	{
		if(Current < Min)
