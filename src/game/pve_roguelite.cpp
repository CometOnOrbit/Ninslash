...
const CPveOperationDef gs_aOperations[NUM_PVE_OPERATIONS] = {
	{PVE_OPERATION_CIRCUIT_BREAKER, "Circuit Breaker", "Disable the shield network and destroy its controller.", PVE_MODE_INVASION, 1, 1, 1, 1, 1, 1, 1, {"Disable 2 shield relays", "Defend the overload terminal for 40 seconds", "Defeat the Overseer Core"}, {PVE_OPERATION_TARGET_SHIELD_RELAY, PVE_OPERATION_TARGET_OVERLOAD_TERMINAL, PVE_OPERATION_TARGET_BOSS}, {2, 40, 1}},
	{PVE_OPERATION_FOUNDRY_SHUTDOWN, "Foundry Shutdown", "Stop production and escape with the coolant core.", PVE_MODE_INVASION, 1, 1, 1, 1, 1, 1, 1, {"Destroy 3 assembly nodes", "Carry the coolant core", "Escape the rising acid within 60 seconds"}, {PVE_OPERATION_TARGET_ASSEMBLY_NODE, PVE_OPERATION_TARGET_COOLANT_CORE, PVE_OPERATION_TARGET_EVACUATION}, {3, 1, 60}},
	{PVE_OPERATION_FIRE_CONTROL_PURGE, "Fire-Control Purge", "Break the targeting network and its siege engine.", PVE_MODE_INVASION, 1, 1, 1, 1, 1, 1, 1, {"Destroy 3 targeting beacons under bombardment", "Clear the mechanical elite wave", "Defeat the Siege Engine"}, {PVE_OPERATION_TARGET_TARGETING_BEACON, PVE_OPERATION_TARGET_NONE, PVE_OPERATION_TARGET_BOSS}, {3, 1, 1}},
	{PVE_OPERATION_SIEGE_LINE, "Siege Line", "Hold the line through a coordinated siege section.", PVE_MODE_HORDE, 1, 1, 1, 1, 1, 1, 1, {"Repel the Bulwark assault", "Survive ranged suppression and sabotage", "Defeat the Siege Engine"}, {PVE_OPERATION_TARGET_DEFENSE_AREA, PVE_OPERATION_TARGET_DEFENSE_AREA, PVE_OPERATION_TARGET_BOSS}, {1, 2, 1}},
	{PVE_OPERATION_ASSEMBLY_SURGE, "Assembly Surge", "Break a self-repairing foundry assault.", PVE_MODE_HORDE, 1, 1, 1, 1, 1, 1, 1, {"Repel the Assembler wave", "Destroy 2 foundry nodes", "Survive the mixed wave and defeat the Overseer Core"}, {PVE_OPERATION_TARGET_DEFENSE_AREA, PVE_OPERATION_TARGET_ASSEMBLY_NODE, PVE_OPERATION_TARGET_BOSS}, {1, 2, 1}},
	{PVE_OPERATION_GRID_STORM, "Grid Storm", "Disable field relays before the EMP storm peaks.", PVE_MODE_HORDE, 1, 1, 1, 1, 1, 1, 1, {"Capture the first relay", "Capture the remaining 2 relays", "Defend inside the rotating EMP field"}, {PVE_OPERATION_TARGET_SHIELD_RELAY, PVE_OPERATION_TARGET_SHIELD_RELAY, PVE_OPERATION_TARGET_DEFENSE_AREA}, {1, 2, 1}},
	{PVE_OPERATION_CORE_RECOVERY, "Core Recovery", "Recover the route data and upload it before extraction.", PVE_MODE_EXTRACTION, 1, 1, 1, 1, 1, 1, 1, {"Carry 2 data cores", "Defend the upload point for 35 seconds", "Reach extraction under high pressure"}, {PVE_OPERATION_TARGET_DATA_CORE, PVE_OPERATION_TARGET_UPLOAD_POINT, PVE_OPERATION_TARGET_EVACUATION}, {2, 35, 1}},
	{PVE_OPERATION_LOCKDOWN_BREAK, "Lockdown Break", "Break the shield lockdown and escape its reinforcements.", PVE_MODE_EXTRACTION, 1, 1, 1, 1, 1, 1, 1, {"Destroy 3 shield nodes", "Defeat the Overseer Core", "Reach extraction through shielded reinforcements"}, {PVE_OPERATION_TARGET_SHIELD_NODE, PVE_OPERATION_TARGET_BOSS, PVE_OPERATION_TARGET_EVACUATION}, {3, 1, 1}},
	{PVE_OPERATION_SIEGE_ROUTE, "Siege Route", "Destroy the siege force and carry its power core out.", PVE_MODE_EXTRACTION, 1, 1, 1, 1, 1, 1, 1, {"Clear the targeting beacons", "Defeat the Siege Engine", "Carry its energy core and hold extraction"}, {PVE_OPERATION_TARGET_TARGETING_BEACON, PVE_OPERATION_TARGET_BOSS, PVE_OPERATION_TARGET_ENERGY_CORE}, {3, 1, 1}},
};
