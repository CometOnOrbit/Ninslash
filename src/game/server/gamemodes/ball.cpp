...
bool CGameControllerBall::InMapArea(int Team, vec2 Pos)
{
	...
	if(Pos.x > m_GoalArea[Team].x && Pos.y > m_GoalArea[Team].y && Pos.x < m_GoalArea[Team].z &&
	   Pos.y < m_GoalArea[Team].w)
		return true;
...
void CGameControllerBall::Tick()
{
	...
	if(!m_RoundEndTick)
	{
		if(InMapArea(TEAM_RED, m_pBall->GetPosition()))
		{
			...
			if(m_LastBallToucher >= 0)
				GameServer()->SendBroadcastFormat(
					-1, false, "%s scores for blue team!", Server()->ClientName(m_LastBallToucher));
			else
		...
		if(InMapArea(TEAM_BLUE, m_pBall->GetPosition()))
		{
			...
			if(m_LastBallToucher >= 0)
				GameServer()->SendBroadcastFormat(
					-1, false, "%s scores for red team!", Server()->ClientName(m_LastBallToucher));
			else
