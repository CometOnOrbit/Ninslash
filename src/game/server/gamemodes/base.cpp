			...
			switch(level)
			{
				...
				case 3:
					if(frandom() < 0.5f)
						...
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_MELEE, PART2_MELEE2)));
					break;
				...
				case 4:
					if(frandom() < 0.5f)
						...
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE3, PART2_BARREL4)));
					else
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE3, PART2_BARREL2)));
					break;
				...
				case 5:
					if(frandom() < 0.5f)
						...
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE1, PART2_BARREL2)));
					else
				...
				case 6:
					if(frandom() < 0.5f)
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Static(SW_CLUSTER, 1)));
					else if(frandom() < 0.5f)
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Static(SW_BOUNCER)));
					else
						pChr->GiveWeapon(
							GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE1, PART2_BARREL1, 2)));
					if(frandom() < 0.3f)
				...
				case 7:
					if(frandom() < 0.3f)
						pChr->GiveWeapon(
							GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE4, PART2_BARREL3, 3)));
					if(frandom() < 0.3f)
						pChr->GiveWeapon(GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_SPIN, PART2_MELEE2)));
					else if(frandom() < 0.3f)
						pChr->GiveWeapon(
							GameServer()->NewWeapon(CWeaponCatalog::Modular(PART1_BASE4, PART2_BARREL4, 3)));
					else if(frandom() < 0.3f)
...
int CGameControllerBase::OnCharacterDeath(class CCharacter *pVictim,
...
void CGameControllerBase::Tick()
{
	...
	else
	{
		...
		// wave win
		if(!m_RoundOverTick && m_Bosses <= 0 && m_Deaths <= 4 && !CountBotsAlive() && CountPlayersAlive(-1, true) > 0 &&
		   !m_WaveStartTick)
		{
			// next wave
			m_WaveStartTick = Server()->Tick() + Server()->TickSpeed() * 15.0f;
		...
		// lose => restart
		if(m_RoundOverTick && !m_GameOverBroadcast &&
		   m_RoundOverTick < Server()->Tick() - Server()->TickSpeed() * 3.0f && m_Wave <= 10)
		{
			m_GameOverBroadcast = true;
			GameServer()->SendBroadcastFormat(-1, false, "Game over on wave %d", m_Wave);
		}
