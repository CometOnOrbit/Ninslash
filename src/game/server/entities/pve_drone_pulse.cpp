...
CPveDronePulse::CPveDronePulse(CGameWorld *pGameWorld, vec2 From, vec2 To, const CAttackSource &Source)
	: CEntity(pGameWorld, CGameWorld::ENTTYPE_PROJECTILE), m_From(From), m_To(To), m_Source(Source),
	  m_StartTick(Server()->Tick()), m_EndTick(Server()->Tick() + max(2, Server()->TickSpeed() / 5))
{
	m_Pos = From;
...
void CPveDronePulse::Snap(int SnappingClient)
{
	if(NetworkClipped(SnappingClient))
		return;
	CNetObj_Projectile *pObj = static_cast<CNetObj_Projectile *>(
