...
class CRepairOrb : public CEntity
{
  ...
  public:
	CRepairOrb(CGameWorld *pWorld, vec2 Pos, CDroid *pTarget)
		: CEntity(pWorld, CGameWorld::ENTTYPE_PROJECTILE), m_pTarget(pTarget),
		  m_ExpireTick(Server()->Tick() + Server()->TickSpeed() * 5)
	{
		m_Pos = Pos;
		m_ProximityRadius = 8;
		s_Alive++;
	...
	void Tick() override
	{
		if(Server()->Tick() >= m_ExpireTick || !TargetAlive())
		{
		...
		if(length(To) < 24.0f)
		{
			m_pTarget->m_Health = min(m_pTarget->m_MaxHealth, m_pTarget->m_Health + 60);
			GameServer()->CreateRepairInd(m_Pos);
			GameWorld()->DestroyEntity(this);
			return;
		}
...
void CAssembler::AbilityTick()
{
	...
	if(m_Status == DROIDSTATUS_ELECTRIC || Server()->Tick() < m_DamageTakenTick + Server()->TickSpeed() * 2)
	{
	...
	float Lowest = 1.0f;
	for(int i = 0; i < Num; i++)
		if(apDroids[i] && apDroids[i] != this && apDroids[i]->m_Health > 0 &&
		   apDroids[i]->m_Health < apDroids[i]->m_MaxHealth)
		{
			float Ratio = (float)apDroids[i]->m_Health / apDroids[i]->m_MaxHealth;
			if(Ratio < Lowest)
			{
				Lowest = Ratio;
				pMostDamaged = apDroids[i];
			}
		}
	if(pMostDamaged)
	{
