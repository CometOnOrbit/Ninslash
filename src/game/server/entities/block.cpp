...
CBlock::CBlock(CGameWorld *pGameWorld, int Type, vec2 Pos) : CEntity(pGameWorld, CGameWorld::ENTTYPE_BLOCK)
{
...
void CBlock::Snap(int SnappingClient)
{
	...
	CNetObj_Block *pP =
		static_cast<CNetObj_Block *>(Server()->SnapNewItem(NETOBJTYPE_BLOCK, m_ID, sizeof(CNetObj_Block)));
	if(!pP)
		return;
