...
CDeathray::CDeathray(CGameWorld *pGameWorld, vec2 Pos) : CBuilding(pGameWorld, Pos, BUILDING_LAZER, TEAM_NEUTRAL)
{
	m_ProximityRadius = TurretPhysSize;
	m_Life = 100;
	m_AttackTick =
		Server()->Tick() - Server()->TickSpeed() * 3.4f + int(m_Pos.x / 4) % int(Server()->TickSpeed() * 3.4f);
	m_Loading = true;
	m_Height = 0;
}
...
void CDeathray::Tick()
{
	...
	if(m_AttackTick > GameServer()->Server()->Tick() - Server()->TickSpeed() * 0.2f)
	{
		vec2 At;
		CCharacter *pHit =
			GameServer()->m_World.IntersectCharacter(m_Pos + vec2(0, 32), m_Pos + vec2(0, m_Height), 4.0f, At);

		if(pHit)
			pHit->Deathray();

		CBuilding *pBHit =
			GameServer()->m_World.IntersectBuilding(m_Pos + vec2(0, 48), m_Pos + vec2(0, m_Height), 4.0f, At, -666);
