...
CBossCrawler::CBossCrawler(CGameWorld *pGameWorld, vec2 Pos) : CDroid(pGameWorld, Pos, DROIDTYPE_BOSSCRAWLER)
{
...
bool CBossCrawler::TickControlled()
{
	CDroidCrawlerControl Control = DroidCrawlerControlBoss();
	...
	if(m_Health <= 0)
	{
		if(Server()->Tick() > m_DamageTakenTick + 70)
		{
			if(Server()->Tick() > m_DamageTakenTick + 140 || abs(m_Vel.y) < 0.2f)
			{
				...
				GameServer()->m_pController->DropWeapon(m_Pos,
														vec2(frandom() * 8.0 - frandom() * 8.0, 0 - frandom() * 14.0),
	...
	if(GameServer()->Collision()->IntersectLine(m_Pos, To, 0x0, &To, false, true))
	{
		...
		if(m_Status != DROIDSTATUS_ELECTRIC && !m_JumpTick &&
		   (frandom() < 0.01f || (abs(m_Vel.x) < 0.15f && frandom() < 0.4f) ||
			(abs(m_Target.x) > 400 && frandom() < 0.05f)))
		{
