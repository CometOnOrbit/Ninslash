	...
	if (m_Health <= 0)
	{
		if (Server()->Tick() > m_DamageTakenTick+70)
		{
			if (Server()->Tick() > m_DamageTakenTick+140 || abs(m_Vel.y) < 0.2f)
			{
				...
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(GetStaticWeapon(SW_UPGRADE)));
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(GetStaticWeapon(SW_UPGRADE)));
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(GetStaticWeapon(SW_RESPAWNER)));
