...
CBossWalker::CBossWalker(CGameWorld *pGameWorld, vec2 Pos) : CDroid(pGameWorld, Pos, DROIDTYPE_BOSSWALKER)
{
...
void CBossWalker::Reset()
{
	...
	m_Vel = vec2(0, 0);
	m_FlyTargetTick = 0;
	m_Mode = CBossWalker::WALKER;
	m_ProximityRadius = BossWalkerPhysSize;
	m_FireDelay = 0;
	m_FireCount = 0;
	m_AttackTimer = 0;
}
...
void CBossWalker::TakeDamage(vec2 Force, int Dmg, const CAttackSource &Source, vec2 Pos)
{
	...
	if(m_Health <= 0)
	{
		...
		GameServer()->m_pController->DropWeapon(m_Pos + vec2(0, -42),
...
void CBossWalker::Tick()
{
	...
	else
	{
		...
		// takeoff
		/*
		if (m_Mode == CBossWalker::WALKER && m_FlyTargetTick > 0 && m_FlyTargetTick < Server()->Tick())
		{
			m_Mode = CBossWalker::DRONE;
			m_Pos.y -= 54;
			m_Vel = vec2(0, -8);
			m_Center = vec2(0, 0);
	...
	// get moving
	if(m_State == CBossWalker::IDLE && m_State == m_NextState && m_StateChangeTick < Server()->Tick())
		SetState(CBossWalker::MOVE);

	m_Anim = 0;

	// fly
	if(m_Mode == CBossWalker::DRONE)
	{
		m_Anim = 3;
	...
	// move
	if(m_State == CBossWalker::MOVE && m_Mode == CBossWalker::WALKER)
	{
		m_Anim = 1;

		float Speed =
			6.0f * (GameServer()->m_pPveDirector ? GameServer()->m_pPveDirector->EnemySpeedMultiplier() : 1.0f);
	...
	// turn
	if(m_State == CBossWalker::TURN)
	{
		m_Dir *= -1;
		m_State = CBossWalker::MOVE;
		m_NextState = CBossWalker::MOVE;
	}
