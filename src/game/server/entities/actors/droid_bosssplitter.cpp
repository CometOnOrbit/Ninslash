...
#include "droid_bosssplitter.h"
#include "droid_crawler.h"


CBossSplitter::CBossSplitter(CGameWorld *pGameWorld, vec2 Pos)
: CDroid(pGameWorld, Pos, DROIDTYPE_BOSSSPLITTER)
{
...
void CBossSplitter::Reset()
{
	...
	m_FlyTargetTick = 0;
	m_Mode = 0;
	m_ProximityRadius = BossSplitterPhysSize;
	m_FireDelay = 0;
	m_FireCount = 0;
	m_AttackTimer = 0;
...
void CBossSplitter::Tick()
{
	...
	if (GameServer()->Collision()->IntersectLine(m_Pos, To, 0x0, &To, false, true))
	{
		...
		if (m_Status != DROIDSTATUS_ELECTRIC && !m_JumpTick && (frandom() < 0.01f || (abs(m_Vel.x) < 0.15f && frandom() < 0.4f) || (abs(m_Target.x) > 300 && frandom() < 0.05f)))
		{
