...
CBossStar::CBossStar(CGameWorld *pGameWorld, vec2 Pos)
: CDroid(pGameWorld, Pos, DROIDTYPE_BOSSSTAR)
{
...
void CBossStar::Reset()
{
	...
	m_FlyTargetTick = 0;
	m_Mode = 0;
	m_ProximityRadius = BossStarPhysSize;
	m_FireDelay = 0;
	m_FireCount = 0;
	m_AttackTimer = 0;
...
void CBossStar::TakeDamage(vec2 Force, int Dmg, const CAttackSource &Source, vec2 Pos)
{
...
void CBossStar::Fire()
{
	if (m_ReloadTimer-- < 0)
	{
		...
		// fan burst
		vec2 Dir = normalize(m_Target*-1);
		float Base = GetAngle(Dir);
		for(int i = -2; i <= 2; i++)
		{
			if(i == 0) continue;
			float A = Base + i * 0.18f;
			vec2 D = vec2(cosf(A), sinf(A));
			GameServer()->CreateProjectile(CAttackSource::Droid(NEUTRAL_BASE, m_Type), 0, TurretPos+D*30.0f, D, TurretPos);
		}
