...
#include <game/server/entities/combat/staticlaser.h>
#include "droid_bosscrawler.h"


CBossCrawler::CBossCrawler(CGameWorld *pGameWorld, vec2 Pos)
: CDroid(pGameWorld, Pos, DROIDTYPE_BOSSCRAWLER)
{
...
void CBossCrawler::Reset()
{
	...
	m_FlyTargetTick = 0;
	m_Mode = 0;
	m_ProximityRadius = BossCrawlerPhysSize;
	m_FireDelay = 0;
	m_FireCount = 0;
	m_AttackTimer = 0;
...
void CBossCrawler::TakeDamage(vec2 Force, int Dmg, const CAttackSource &Source, vec2 Pos)
{
...
void CBossCrawler::Move()
{
	
}

void CBossCrawler::Tick()
{
	...
	if (m_Health <= 0)
	{
		if (Server()->Tick() > m_DamageTakenTick+70)
		{
			if (Server()->Tick() > m_DamageTakenTick+140 || abs(m_Vel.y) < 0.2f)
			{
				...
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(CWeaponCatalog::Static(SW_UPGRADE)));
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(CWeaponCatalog::Static(SW_UPGRADE)));
				GameServer()->m_pController->DropWeapon(m_Pos, vec2(frandom()*8.0-frandom()*8.0, 0-frandom()*14.0), GameServer()->NewWeapon(CWeaponCatalog::Static(SW_RESPAWNER)));
	...
	if (GameServer()->Collision()->IntersectLine(m_Pos, To, 0x0, &To, false, true))
	{
		...
		{
			//m_Vel.y = -16;
			if (abs(m_Target.x) > 400)
				m_JumpForce = -7.0f;
			else
				m_JumpForce = -9.0f-frandom()*3.0f;
		...
		else if (abs(m_Target.x) > 20 && abs(m_Target.x) < 500)
...
bool CBossCrawler::Target()
{
	if (m_TargetIndex >= 0 && m_TargetIndex < MAX_CLIENTS)
	{
		...
		{
			m_Dir *= -1;
			m_State = CBossCrawler::IDLE;
			m_StateChangeTick = Server()->Tick() + Server()->TickSpeed() * (1 + frandom());
		}
		*/

		int Distance = distance(pCharacter->m_Pos, m_Pos);
		if (Distance < 800 && !GameServer()->Collision()->FastIntersectLine(pCharacter->m_Pos+vec2(0, -24), m_Pos))
		{
...
bool CBossCrawler::FindTarget()
{
	...
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if (abs(m_Pos.x - pCharacter->m_Pos.x) < 700 && abs(m_Pos.y - pCharacter->m_Pos.y) < 200)
		{
