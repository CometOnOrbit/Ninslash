...
{
	if((Get(x - 1, y, BACKGROUND) || Get(x + 1, y, BACKGROUND)) &&
	   (Get(x, y - 1, BACKGROUND) || Get(x, y + 1, BACKGROUND)))
		return true;
		...
		for(int y = 4; y < m_Height - 4; y++)
		{
			...
			if(!Get(x, y) && Get(x, y + 1))
			{
				...
				if(x1 + x2 >= 6)
				{
					...
					// avoid door
					if(x - (x1 + 1) < m_EndPos.x && m_EndPos.x < x + x2 + 1 && abs(m_EndPos.y - y) < 2)
						Valid = false;
	...
	for(int x = 2; x < m_Width - 2; x++)
		for(int y = 2; y < m_Height - 2; y++)
		{
			...
			if(Get(x, y) && !Get(x, y - 1) && !Get(x, y + 1) &&
			   ((!Get(x - 1, y) && Get(x + 1, y) && Get(x + 1, y - 1) && Get(x + 1, y + 1)) ||
				((!Get(x + 1, y) && Get(x - 1, y) && Get(x - 1, y - 1) && Get(x - 1, y + 1)))))
				Set(0, x, y);
		}
}
...
void CGenLayer::Scan()
{
	...
	for(int x = 1; x < m_Width - 1; x++)
		...
		{
			// bot left
			if(!Used(x, y) && !Used(x + 1, y) && !Used(x, y - 1) && Get(x - 1, y) && Get(x - 1, y + 1) && Get(x, y + 1))
			{
				int px = x + 1;
				int py = y - 1;
				int py2 = y - 1;
		   ...
		   InvasionThemeFromLevel(g_Config.m_SvMapGenLevel) == INVASION_THEME_ACID_ESCAPE)
		{
			for(int y = m_Height - 2; y > 2; y--)
				for(int x = 2; x < m_Width - 2; x++)
				{
					...
					{
						if(!Used(x - 1, y) && !Used(x, y) && !Used(x + 1, y) && Get(x - 1, y + 1) && Get(x, y + 1) &&
						   Get(x + 1, y + 1))
						{
	...
	for(int x = 2; x < m_Width - 2; x++)
		for(int y = 2; y < m_Height - 2; y++)
		{
			if(!Used(x - 2, y) && !Used(x - 1, y) && !Used(x, y) && !Used(x + 1, y) && !Used(x + 2, y) &&
			   Get(x - 2, y + 1) && Get(x - 1, y + 1) && Get(x, y + 1) && Get(x + 1, y + 1) && Get(x + 2, y + 1) &&
			   !IsNearSlope(x, y + 1))
			{
	...
	for(int x = 2; x < m_Width - 2; x++)
		for(int y = 2; y < m_Height - 2; y++)
		{
			...
			if(!Used(x, y) && !Used(x + 1, y) && !Used(x, y + 1) && !Used(x + 2, y) && !Used(x, y + 2) &&
			   Get(x - 1, y) && Get(x - 1, y + 1) && Get(x - 1, y + 2) && Get(x, y - 1) && Get(x + 1, y - 1) &&
			   Get(x + 2, y - 1))
			{
	...
	for(int x = 2; x < m_Width - 2; x++)
		...
		{
			// top left
			if(Get(x, y) && Get(x - 1, y) && Get(x, y + 1) && !Used(x + 1, y) && !Used(x + 1, y - 1) && !Used(x, y - 1))
			{
