...
void CPlayer::SendForgeResult(int Result,
							  int Operation,
							  int TargetSlot,
							  int MaterialSlot,
							  int Cost,
	...
	*/
	...
	if(!GameServer()->m_World.m_Paused)
	{
		if(!m_pCharacter && m_Team == TEAM_SPECTATORS && m_SpectatorID == SPEC_FREEVIEW)
			m_ViewPos -= vec2(clamp(m_ViewPos.x - m_LatestActivity.m_TargetX, -500.0f, 500.0f),
							  clamp(m_ViewPos.y - m_LatestActivity.m_TargetY, -400.0f, 400.0f));

		// dead in survival mode
		if(m_Team != TEAM_SPECTATORS && g_Config.m_SvSurvivalMode && !m_pCharacter &&
		   m_DieTick < Server()->Tick() - Server()->TickSpeed() * 1)
			m_ViewPos -= vec2(clamp(m_ViewPos.x - m_LatestActivity.m_TargetX, -500.0f, 500.0f),
							  clamp(m_ViewPos.y - m_LatestActivity.m_TargetY, -400.0f, 400.0f));
...
void CPlayer::Snap(int SnappingClient)
{
	...
	CNetObj_ClientInfo *pClientInfo = static_cast<CNetObj_ClientInfo *>(
		Server()->SnapNewItem(NETOBJTYPE_CLIENTINFO, m_ClientID, sizeof(CNetObj_ClientInfo)));
	if(!pClientInfo)
		return;
	...
	pPlayerInfo->m_Latency =
		SnappingClient == -1 ? m_Latency.m_Min : GameServer()->m_apPlayers[SnappingClient]->m_aActLatency[m_ClientID];
	...
	int *apLevels[] = {&pPlayerInfo->m_Weapon1Level,
	...
	if(m_ClientID == SnappingClient && Spectating())
	{
		...
		CNetObj_SpectatorInfo *pSpectatorInfo = static_cast<CNetObj_SpectatorInfo *>(
			Server()->SnapNewItem(NETOBJTYPE_SPECTATORINFO, m_ClientID, sizeof(CNetObj_SpectatorInfo)));
		if(!pSpectatorInfo)
			return;
...
void CPlayer::OnDisconnect(const char *pReason)
{
	...
	if(Server()->ClientIngame(m_ClientID))
	{
		...
		{
			if(pReason && *pReason)
				GameServer()->SendChatTarget(
					-1, "'%s' has left the game (%s)", Server()->ClientName(m_ClientID), pReason);
			else
...
void CPlayer::OnDirectInput(CNetObj_PlayerInput *NewInput)
{
	...
	if(NewInput->m_Direction || m_LatestActivity.m_TargetX != NewInput->m_TargetX ||
	   m_LatestActivity.m_TargetY != NewInput->m_TargetY || NewInput->m_Jump || NewInput->m_Fire & 1 ||
	   NewInput->m_Hook || NewInput->m_Down)
	{
...
void CPlayer::SetTeam(int Team, bool DoChatMsg)
{
	...
	str_format(
		aBuf, sizeof(aBuf), "team_join player='%d:%s' m_Team=%d", m_ClientID, Server()->ClientName(m_ClientID), m_Team);
