...
bool CAIdef::FindShop()
{
	...
	int Num =
		GameServer()->m_World.FindEntities(m_LastPos, 4000, (CEntity **)apEnts, 128, CGameWorld::ENTTYPE_BUILDING);
		...
		if(Player()->GetCharacter() && Player()->GetCharacter()->IsBombCarrier())
		{
			if(GameServer()->m_pController->InBombArea(m_Pos))
			{
				...
				if(pWeapon && pWeapon->GetWeaponProfile().m_Definition.m_Kind == EWeaponDefinitionKind::Static &&
				   pWeapon->GetWeaponProfile().m_Definition.m_StaticType == SW_BOMB)
				{
					m_Attack = 1;
					m_Move = 0;
					m_Hook = 0;
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							m_NextTargetTick = GameServer()->Server()->Tick() +
											   GameServer()->Server()->TickSpeed() * (5.0f + frandom() * 10.0f);
