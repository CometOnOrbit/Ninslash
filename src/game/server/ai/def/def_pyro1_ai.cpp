...
#include "def_pyro1_ai.h"

CAIdefpyro1::CAIdefpyro1(CGameContext *pGameServer, CCharacter *pCharacter) : CAI(pGameServer, pCharacter)
{
	m_SkipMoveUpdate = 0;
	Player()->SetCustomSkin(9);
	m_StartPos = vec2(0, 0);
	m_ShockTimer = 0;
}

void CAIdefpyro1::OnCharacterSpawn(CCharacter *pChr)
{
...
void CAIdefpyro1::ReceiveDamage(int CID, int Dmg)
{
	// m_ShockTimer = 10;
	m_Attack = 0;
}

void CAIdefpyro1::DoBehavior()
{
	m_Attack = 0;
	...
	if((Shooting && Player()->GetCharacter()->IsGrounded()) ||
	   (abs(m_Pos.x - m_TargetPos.x) < 40 && abs(m_Pos.y - m_TargetPos.y) < 40))
	{
		// stand still
		m_Move = 0;
		m_Jump = 0;
