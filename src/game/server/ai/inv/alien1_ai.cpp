...
CAIalien1::CAIalien1(CGameContext *pGameServer, CPlayer *pPlayer, int Level) : CAI(pGameServer, pPlayer)
{
	m_SkipMoveUpdate = 0;
	m_StartPos = vec2(0, 0);
	m_ShockTimer = 0;
...
void CAIalien1::OnCharacterSpawn(CCharacter *pChr)
{
	...
	if(m_Skin == SKIN_ALIEN3)
	{
		...
		pChr->SetArmor(60 + min((m_Level - 1) * 3, 300));
		m_PowerLevel = 8;
	...
	else if(m_Skin == SKIN_ALIEN4)
	{
		...
		pChr->SetHealth(60 + min((m_Level - 1) * 3, 200));
		pChr->SetArmor(60 + min((m_Level - 1) * 3, 350));
		m_PowerLevel = 12;
		m_TriggerLevel = 15 + rand() % 5;
	}
	else if(m_Skin == SKIN_ALIEN5)
	{
		...
		pChr->SetHealth(50 + min((m_Level - 1) * 3, 150));
		pChr->SetArmor(60 + min((m_Level - 1) * 3, 300));
		m_PowerLevel = 10;
		m_TriggerLevel = 15 + rand() % 5;
	}
