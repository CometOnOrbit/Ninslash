...
#include <game/shared/core/gamecore.h> // get_angle
#include <game/shared/pve/pve_roguelite.h>
...
#include <game/client/render/animdata.h>


#include <game/client/state/customstuff.h>
#include <game/client/state/playerinfo.h>

#include <game/client/effects/flow.h>
#include <game/client/resources/skins.h>
#include <game/client/effects/effects.h>
#include <game/client/ui/pve_roguelite.h>
#include <game/client/effects/tracer.h>
#include <game/client/resources/sounds.h>
#include <game/client/input/controls.h>
		...
		if (WeaponStaticType == SW_FLAMER)
		{
			...
			if (pCustomPlayerInfo->m_FlameState > 0)
			{
				...
				for (int i = 0; i <= Slices; i++)
				{
					...
					bool NoCol = true;
					if (Collision()->IntersectLine(StartPos, p1, 0x0, &p1)) NoCol = false;
					if (Collision()->IntersectLine(StartPos, p3, 0x0, &p3)) NoCol = false;
					if (Collision()->IntersectLine(StartPos, p2, 0x0, &p2)) NoCol = false;
					if (Collision()->IntersectLine(StartPos, p4, 0x0, &p4)) NoCol = false;
					
					if (NoCol)
						m_pClient->m_pEffects->SimpleLight((p1+p2+p3+p4)/4+vec2(0, 8), vec4(1.0f, 0.75f, 0.5f, 0.3f), 60+i*6);
				...
				//m_pClient->m_pEffects->BoxLight((LightStartPos+FinalPos)/2-DirY*OffsetY*(Flip?1:0), vec4(1.0f, 0.75f, 0.5f, 0.4f), vec2(80, length(LightStartPos-FinalPos)+20), atan2(Dir.y, Dir.x)+pi/2);
