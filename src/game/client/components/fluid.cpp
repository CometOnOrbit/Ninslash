					...
					if(abs(Pos.y - m_aPool[i].m_Pos.y + m_aPool[i].m_aSurfaceY[x]) < 16)
					{
						...
						*/
						...
						if(abs(m_aPool[i].m_aSurfaceVel[x] - Vel.y) > 3.0f)
							m_pClient->m_pEffects->Acid(vec2(Pos.x, m_aPool[i].m_Pos.y + m_aPool[i].m_aSurfaceY[x]),
														vec2((frandom() - frandom()) * 0.4f, -frandom() * 0.7f));
				...
				if(abs(Pos.y - m_GlobalPool.m_Pos.y + m_GlobalPool.m_aSurfaceY[x]) < 16)
				{
					...
					*/
					...
					if(abs(m_GlobalPool.m_aSurfaceVel[x] - Vel.y) > 3.0f)
						m_pClient->m_pEffects->Acid(vec2(Pos.x, m_GlobalPool.m_Pos.y + m_GlobalPool.m_aSurfaceY[x]),
													vec2((frandom() - frandom()) * 0.4f, -frandom() * 0.7f));
				}
			}
		}
...
void CFluid::RenderGlobalAcid()
{
	...
	{
		IGraphics::CFreeformItem Freeform(Center.x - ScreenX / 2,
										  Center.y - ScreenY / 2,
										  Center.x + ScreenX / 2,
										  Center.y - ScreenY / 2,
										  Center.x - ScreenX / 2,
	...
	else
	{
		...
		for(int f = -3; f < Size.x / 16; f++)
		{
			...
			IGraphics::CFreeformItem Freeform(Pos.x + f * StepX + offX,
											  y1,
											  Pos.x + (f + 1) * StepX + offX,
											  y2,
											  Pos.x + f * StepX + offX,
											  Pos.y + Size.y,
											  Pos.x + (f + 1) * StepX + offX,
...
void CFluid::RenderPool(int i)
{
	...
	for(int f = 0; f < Size.x / 16; f++)
	{
		...
		IGraphics::CFreeformItem Freeform(Pos.x + f * StepX,
										  y1,
										  Pos.x + (f + 1) * StepX,
										  y2,
										  Pos.x + f * StepX,
