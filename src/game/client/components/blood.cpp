...
void CBlood::Bounce(vec2 Pos, vec2 Dir, int Group, vec4 Color)
{
	...
	if(g_Config.m_GfxMultiBuffering && frandom() * 10 < 3 && Group == GROUP_BLOOD)
		m_pClient->m_pEffects->Splatter(
			b.m_Pos + Dir * frandom() * 48.0f - Dir * frandom() * 16.0f, b.m_Rot, b.m_StartSize * 2, Color);
}
...
void CBlood::Update(float TimePassed)
{
	...
	for(int g = 0; g < NUM_GROUPS; g++)
	{
		...
		while(i != -1)
		{
			...
			if(OnForceTile != 0)
				Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.99f, 0);
			else
				Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.2f + 0.5f * frandom(), 0);
			// Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.8f, 0);

			/*
			if (m_aBlood[i].m_Spr >= SPRITE_BONE01)
				Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.5f+0.5f*frandom(), 0);
			else
			if (m_aBlood[i].m_Spr > SPRITE_BLOOD01)
				Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.6f+0.6f*frandom(), 0);
			else
				Collision()->MovePoint(&m_aBlood[i].m_Pos, &Vel, 0.1f+0.8f*frandom(), 0);
			*/

			// break big blood splats into smaller ones
			if(m_aBlood[i].m_Spr != SPRITE_BLOOD01 && m_aBlood[i].m_Spr < SPRITE_BONE01)
			{
				if((Vel.y < 0 && OldVel.y > 0) || (Vel.y > 0 && OldVel.y < 0) || (Vel.x < 0 && OldVel.x > 0) ||
				   (Vel.x > 0 && OldVel.x < 0))
				{
			...
			if(!Collision()->IsTileSolid(P.x, P.y + 16))
			{
				...
				else if(Collision()->IsTileSolid(P.x + 16, P.y))
				{
					float Dist = abs(P.x - int((P.x + 16) / 32) * 32.0f);
...
void CBlood::RenderGroup(int Group)
{
	...
	else if(Group == GROUP_BLOOD)
	{
		...
		while(i != -1)
		{
			...
			Graphics()->SetColor(m_aBlood[i].m_Color.r, m_aBlood[i].m_Color.g, m_aBlood[i].m_Color.b, 1.2f - a);
