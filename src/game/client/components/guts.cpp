			...
			while(i != -1)
			{
				...
				for(int p = 0; p < Parts; p++)
				{
					...
					if(p == 0)
					{
						...
						//	m_pClient->m_pEffects->Blood(m_aGuts[i].m_aPos[p], normalize(n)*4.0f +
						// m_aGuts[i].m_aVel[p]/2, m_aGuts[i].m_Color);
					}
					else if(p == Parts - 1)
					{
...
void CGuts::RenderGroup(int Group)
{
	if(Group == GROUP_GUTS)
	{
		...
		while(i != -1)
		{
			...
			Graphics()->SetColor(m_aGuts[i].m_Color.r * 0.6f + 0.4f,
								 m_aGuts[i].m_Color.g * 0.6f + 0.4f,
								 m_aGuts[i].m_Color.b * 0.7f + 0.3f,
								 a);
			...
			for(int t = 1; t < (Parts - 1) * 2 + 1; t++)
			{
				...
				Graphics()->QuadsSetSubsetFree((t - 1) / Frames,
											   m_aGuts[i].m_Spr * 0.5f,
											   (t) / Frames,
											   m_aGuts[i].m_Spr * 0.5f,
											   (t - 1) / Frames,
											   m_aGuts[i].m_Spr * 0.5f + 0.5f,
											   (t) / Frames,
											   m_aGuts[i].m_Spr * 0.5f + 0.5f);
