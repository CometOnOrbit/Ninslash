...
int CMenus::DoButton_DemoPlayer(const void *pID, const char *pText, int Checked, const CUIRect *pRect)
{
	RenderTools()->DrawUIRect(
		pRect, vec4(0.95f, 0.58f, 0.18f, Checked ? 0.10f : 0.38f) * ButtonColorMul(pID), CUI::CORNER_ALL, 5.0f);
...
{
	RenderTools()->DrawUIRect(pRect,
							  Checked ? vec4(0.12f, 0.13f, 0.16f, 0.10f)
									  : vec4(0.95f, 0.58f, 0.18f, 0.38f) * ButtonColorMul(pID),
							  Corners,
							  5.0f);
...
void CMenus::RenderDemoPlayer(CUIRect MainView)
{
	...
	// do seekbar
	{
		...
		for(int i = 0; i < pInfo->m_NumTimelineMarkers; i++)
		{
			...
			IGraphics::CQuadItem QuadItem(
				SeekBar.x + (SeekBar.w - 10.0f) * Ratio, SeekBar.y, UI()->PixelSize(), SeekBar.h);
			Graphics()->QuadsDrawTL(&QuadItem, 1);
			Graphics()->QuadsEnd();
		}
		...
		str_format(aBuffer,
				   sizeof(aBuffer),
				   "%d:%02d / %d:%02d",
				   CurrentTick / SERVER_TICK_SPEED / 60,
				   (CurrentTick / SERVER_TICK_SPEED) % 60,
				   TotalTicks / SERVER_TICK_SPEED / 60,
				   (TotalTicks / SERVER_TICK_SPEED) % 60);
	...
	if(m_MenuActive)
	{
		...
		{
			if(DoButton_Sprite(
				   &s_PlayPauseButton, IMAGE_DEMOBUTTONS, SPRITE_DEMOBUTTON_PAUSE, false, &Button, CUI::CORNER_ALL))
				DemoPlayer()->Pause();
		}
		else
		{
			if(DoButton_Sprite(
				   &s_PlayPauseButton, IMAGE_DEMOBUTTONS, SPRITE_DEMOBUTTON_PLAY, false, &Button, CUI::CORNER_ALL))
				DemoPlayer()->Unpause();
		}
		...
		// jump bar: quick jumps + time input
		{
			...
			if(DoButton_DemoPlayer(&s_JumpBack1m, "<1m", 0, &JumpBtn) && TotalTicks > 0)
			{
				...
				if(targetTick < pInfo->m_FirstTick)
					targetTick = pInfo->m_FirstTick;
				m_pClient->OnReset();
				m_pClient->m_SuppressEvents = true;
			...
			{
				int targetTick = pInfo->m_CurrentTick + 60 * SERVER_TICK_SPEED;
				if(targetTick > pInfo->m_LastTick)
					targetTick = pInfo->m_LastTick;
				m_pClient->OnReset();
				m_pClient->m_SuppressEvents = true;
				DemoPlayer()->SetPos((float)(targetTick - pInfo->m_FirstTick) / (float)TotalTicks);
				m_pClient->m_SuppressEvents = false;
	...
	if(IncreaseDemoSpeed || Input()->KeyPresses(KEY_MOUSE_WHEEL_UP))
	{
		...
		else if(pInfo->m_Speed < 0.25f)
			DemoPlayer()->SetSpeed(0.25f);
		else if(pInfo->m_Speed < 0.5f)
			DemoPlayer()->SetSpeed(0.5f);
		else if(pInfo->m_Speed < 0.75f)
			DemoPlayer()->SetSpeed(0.75f);
		else if(pInfo->m_Speed < 1.0f)
			DemoPlayer()->SetSpeed(1.0f);
		else if(pInfo->m_Speed < 2.0f)
			DemoPlayer()->SetSpeed(2.0f);
		else if(pInfo->m_Speed < 4.0f)
	...
	else if(DecreaseDemoSpeed || Input()->KeyPresses(KEY_MOUSE_WHEEL_DOWN))
	{
		if(pInfo->m_Speed > 4.0f)
			...
			DemoPlayer()->SetSpeed(0.25f);
		else if(pInfo->m_Speed > 0.1f)
			DemoPlayer()->SetSpeed(0.1f);
		else
			DemoPlayer()->SetSpeed(0.05f);
	}
}
...
CMenus::CListboxItem CMenus::UiDoListboxNextRow()
{
	...
	s_RowView.VSplitLeft(s_RowView.w / (gs_ListBoxItemsPerRow - gs_ListBoxItemIndex % gs_ListBoxItemsPerRow) /
							 (UI()->Scale()),
						 &Item.m_Rect,
						 &s_RowView);
			...
			else
			{
				for(int i = 0; i < m_NumInputEvents; i++)
				{
					...
					if(NewIndex > -1 && NewIndex < gs_ListBoxNumItems)
					{
						// scroll
						float Offset =
							(NewIndex / gs_ListBoxItemsPerRow - gs_ListBoxNewSelected / gs_ListBoxItemsPerRow) *
							gs_ListBoxRowHeight;
						int Scroll = gs_ListBoxOriginalView.y > Item.m_Rect.y + Offset ? -1
									 : gs_ListBoxOriginalView.y + gs_ListBoxOriginalView.h <
											 Item.m_Rect.y + Item.m_Rect.h + Offset
										 ? 1
										 : 0;
						if(Scroll)
						{
							int NumViewable = (int)(gs_ListBoxOriginalView.h / gs_ListBoxRowHeight) + 1;
							int ScrollNum = (gs_ListBoxNumItems + gs_ListBoxItemsPerRow - 1) / gs_ListBoxItemsPerRow -
											NumViewable + 1;
							if(Scroll < 0)
							{
								int Num =
									(gs_ListBoxOriginalView.y - Item.m_Rect.y - Offset + gs_ListBoxRowHeight - 1.0f) /
									gs_ListBoxRowHeight;
								gs_ListBoxScrollTarget -= (1.0f / ScrollNum) * Num;
							}
							else
							{
								int Num = (Item.m_Rect.y + Item.m_Rect.h + Offset -
										   (gs_ListBoxOriginalView.y + gs_ListBoxOriginalView.h) + gs_ListBoxRowHeight -
										   1.0f) /
										  gs_ListBoxRowHeight;
								gs_ListBoxScrollTarget += (1.0f / ScrollNum) * Num;
							}
...
int CMenus::DemolistFetchCallback(const char *pName, int IsDir, int StorageType, void *pUser)
{
	...
	if((pName[0] == '.' &&
		(pName[1] == 0 || (pName[1] == '.' && pName[2] == 0 && !str_comp(pSelf->m_aCurrentDemoFolder, "demos")))) ||
...
void CMenus::RenderDemoList(CUIRect MainView)
{
	...
	if(!m_DemolistSelectedIsDir && m_DemolistSelectedIndex >= 0 && m_lDemos[m_DemolistSelectedIndex].m_Valid)
	{
		...
		int Length = ((m_lDemos[m_DemolistSelectedIndex].m_Info.m_aLength[0] << 24) & 0xFF000000) |
					 ((m_lDemos[m_DemolistSelectedIndex].m_Info.m_aLength[1] << 16) & 0xFF0000) |
					 ((m_lDemos[m_DemolistSelectedIndex].m_Info.m_aLength[2] << 8) & 0xFF00) |
					 (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aLength[3] & 0xFF);
		char aBuf[64];
		...
		unsigned Size = (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapSize[0] << 24) |
						(m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapSize[1] << 16) |
						(m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapSize[2] << 8) |
						(m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapSize[3]);
		...
		unsigned Crc = (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapCrc[0] << 24) |
					   (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapCrc[1] << 16) |
					   (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapCrc[2] << 8) |
					   (m_lDemos[m_DemolistSelectedIndex].m_Info.m_aMapCrc[3]);
	...
	if(DoButton_Menu(&s_PlayButton, m_DemolistSelectedIsDir ? Localize("Open") : Localize("Play"), 0, &PlayRect) ||
	   Activated)
	{
		if(m_DemolistSelectedIndex >= 0)
		{
			if(m_DemolistSelectedIsDir) // folder
			{
				...
				else // sub folder
				{
					...
					str_format(m_aCurrentDemoFolder,
							   sizeof(m_aCurrentDemoFolder),
							   "%s/%s",
							   aTemp,
			...
			else // file
			{
				char aBuf[512];
				str_format(
					aBuf, sizeof(aBuf), "%s/%s", m_aCurrentDemoFolder, m_lDemos[m_DemolistSelectedIndex].m_aFilename);
	...
	if(!m_DemolistSelectedIsDir)
	{
		...
		if(DoButton_Menu(&s_RenderButton, Localize("Render"), 0, &RenderRect))
		{
			if(m_DemolistSelectedIndex >= 0)
			{
				...
				str_format(m_aDemoRenderSource,
						   sizeof(m_aDemoRenderSource),
						   "%s/%s",
