...
void CTracer::Add(int Type, int ItemID, vec2 Pos, vec2 StartPos, int StartTick, float ProjectileSize, vec2 Vel)
{
	...
	while(i != -1)
	{
		if(abs(m_aTracer[i].m_StartTick - StartTick) < 2 && m_aTracer[i].m_ItemID == ItemID &&
		   m_aTracer[i].m_Type == Type)
		{
	...
	if(Group == GROUP_TRACERS)
	{
		...
		while(i != -1)
		{
			...
			for(int t = 1; t < m_aTracer[i].m_Parts; t++)
			{
				...
				if(d > 1.0f && d < 200.0f)
				{
					IGraphics::CColorVertex aColors[4] = {IGraphics::CColorVertex(0,
																				  m_aTracer[i].m_Color.r,
																				  m_aTracer[i].m_Color.g,
																				  m_aTracer[i].m_Color.b,
																				  m_aTracer[i].m_Color.a * a1),
														  IGraphics::CColorVertex(1,
																				  m_aTracer[i].m_Color.r,
																				  m_aTracer[i].m_Color.g,
																				  m_aTracer[i].m_Color.b,
																				  m_aTracer[i].m_Color.a * a2),
					...
					IGraphics::CFreeformItem Freeform1(p1.x - Out1.x,
													   p1.y - Out1.y,
													   p2.x - Out2.x,
													   p2.y - Out2.y,
													   p1.x + Out1.x,
	...
	else if(Group == GROUP_SPRITETRACERS)
	{
		...
		while(i != -1)
		{
			...
			IGraphics::CColorVertex aColors[4] = {
				IGraphics::CColorVertex(0, m_aTracer[i].m_Color.r, m_aTracer[i].m_Color.g, m_aTracer[i].m_Color.b,
			m_aTracer[i].m_Color.a), IGraphics::CColorVertex(1, m_aTracer[i].m_Color.r, m_aTracer[i].m_Color.g,
			m_aTracer[i].m_Color.b, m_aTracer[i].m_Color.a), IGraphics::CColorVertex(2, m_aTracer[i].m_Color.r,
			m_aTracer[i].m_Color.g, m_aTracer[i].m_Color.b, 0.0f), IGraphics::CColorVertex(3, m_aTracer[i].m_Color.r,
			m_aTracer[i].m_Color.g, m_aTracer[i].m_Color.b, 0.0f)}; Graphics()->SetColorVertex(aColors, 4);
			...
			*/
			...
			for(int t = 1; t < m_aTracer[i].m_Parts; t++)
			{
				...
				// RenderTools()->SelectSprite(SPRITE_FX_CHAINSAW1+(CustomStuff()->GetSpriteFrame(6,
				// 4)+m_aTracer[i].m_ItemID)%4,  + (false ? SPRITE_FLAG_FLIP_Y : 0), 0, 0, 1-t1, 1-t2);

				float tt = CustomStuff()->m_SawbladeAngle * m_aTracer[i].m_RotSpeed;

				RenderTools()->SelectSprite(SPRITE_TRAIL1 + m_aTracer[i].m_Sprite,
											0,
											0,
											0,
											(1 - t1) * m_aTracer[i].m_Scale + tt,
											(1 - t2) * m_aTracer[i].m_Scale + tt);
