	...
	for(int g = 0; g < NUM_GROUPS; g++)
	{
		...
		if(g == GROUP_BRAINS)
		{
			while(i != -1)
			{
				...
				if((OnForceTile > 0 && m_aBrains[i].m_aVel[0].x < tresh) ||
				   (OnForceTile < 0 && m_aBrains[i].m_aVel[0].x > -tresh))
				...
				for(int p = 1; p < m_aBrains[i].m_Parts + 1; p++)
					Collision()->IntersectLine(
						m_aBrains[i].m_aPos[0], m_aBrains[i].m_aPos[p], 0x0, &m_aBrains[i].m_aPos[p]);
	...
	if(Group == GROUP_BRAINS)
	{
		...
		*/
		...
		while(i != -1)
		{
			...
			Graphics()->SetColor(m_aBrains[i].m_Color.r * 0.6f + 0.4f,
								 m_aBrains[i].m_Color.g * 0.6f + 0.4f,
								 m_aBrains[i].m_Color.b * 0.7f + 0.3f,
								 a);
			...
			for(int x = 1; x < m_aBrains[i].m_Parts + 1; x++)
			{
				...
				Graphics()->QuadsSetSubsetFree(0.5f,
											   0.5f,
											   0.5f,
											   0.5f,
											   0.5f + sin(a1) * 0.5f,
