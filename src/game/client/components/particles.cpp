...
void CParticles::Update(float TimePassed)
{
	...
	for(int g = 0; g < NUM_GROUPS; g++)
	{
		...
		while(i != -1)
		{
			...
			bool Collided = Collision()->MovePoint(
				&m_aParticles[i].m_Pos, &Vel, 0.1f + 0.9f * frandom(), 0, m_aParticles[i].m_IgnoreCollision);
			...
			if(m_aParticles[i].m_Chunk && Collided)
			{
				...
				if(g_Config.m_GoreBlood > 0 && abs(m_aParticles[i].m_Vel.x) + abs(m_aParticles[i].m_Vel.y) > 340)
				{
					...
					m_pClient->m_pEffects->Blood(
						m_aParticles[i].m_Pos, vec2(frandom() - frandom(), frandom() - frandom()) * 0.2f, Color);
				}
			}
			...
			if(g == GROUP_EXPLOSIONS)
			{
				if(!m_aParticles[i].m_Special)
				{
					...
					CustomStuff()->AddImpact(
						vec4(p.x - r, p.y - r, p.x + r, p.y + r), CCustomStuff::IMPACT_GRENADE, vec2(-0.6f, -0.6f));

					p = m_aParticles[i].m_Pos + vec2(0, -20);
					CustomStuff()->AddImpact(
						vec4(p.x - r, p.y - r, p.x + r, p.y + r), CCustomStuff::IMPACT_GRENADE, vec2(0, -1));
...
{
	{
		...
		while(i != -1)
		{
			...
			Graphics()->SetColor(
				m_aParticles[i].m_Color.r, m_aParticles[i].m_Color.g, m_aParticles[i].m_Color.b, 1 - a);
...
void CParticles::RenderGroup(int Group)
{
	...
	else if(Group == GROUP_LAZER)
	{
		...
		while(i != -1)
		{
			...
			m_pClient->m_pEffects->BoxLight(vec2(p.x, p.y + m_aParticles[i].m_Height / 2),
											vec4(1.0f, 0.75f, 0.5f, 1 - a),
											vec2(70, m_aParticles[i].m_Height + 70));
	...
	else if(Group == GROUP_CRAFTING)
	{
		...
		while(i != -1)
		{
			...
			float Size = mix(m_aParticles[i].m_StartSize, m_aParticles[i].m_EndSize * 1.0f, a);
			Graphics()->SetColor(
				m_aParticles[i].m_Color.r, m_aParticles[i].m_Color.g, m_aParticles[i].m_Color.b, 1.0f - a);
			Graphics()->QuadsSetRotation(m_aParticles[i].m_Rot);
	...
	else if(Group == GROUP_SWORDHITS)
	{
		...
		while(i != -1)
		{
			...
			float Size = mix(m_aParticles[i].m_StartSize, m_aParticles[i].m_EndSize * 1.0f, a);
			RenderTools()->SelectSprite(m_aParticles[i].m_Spr + a * m_aParticles[i].m_Frames,
										m_aParticles[i].m_Flip ? SPRITE_FLAG_FLIP_Y : 0);
			Graphics()->QuadsSetRotation(m_aParticles[i].m_Rot);
			Graphics()->SetColor(m_aParticles[i].m_Color.r, m_aParticles[i].m_Color.g, m_aParticles[i].m_Color.b, 1);
			IGraphics::CQuadItem QuadItem(p.x, p.y, Size, Size);
			Graphics()->QuadsDraw(&QuadItem, 1);
	...
	else if(Group == GROUP_ELECTRIC)
	{
		...
		while(i != -1)
		{
			...
			RenderTools()->SelectSprite(m_aParticles[i].m_Spr + a * m_aParticles[i].m_Frames);
			Graphics()->QuadsSetRotation(m_aParticles[i].m_Rot);
			Graphics()->SetColor(m_aParticles[i].m_Color.r,
	...
	else if(Group == GROUP_SPARKS)
	{
		...
		while(i != -1)
		{
			...
			m_pClient->m_pEffects->SimpleLight(
				m_aParticles[i].m_Pos, vec4(0.6f, 0.8f, 1.0f, 0.5f - a / 2.0f), Size * 1.5f);
	...
	else if(Group == GROUP_EXPLOSIONS)
	{
		...
		while(i != -1)
		{
			...
			m_pClient->m_pEffects->SimpleLight(
				m_aParticles[i].m_Pos, vec4(1.0f, 0.9f, 0.8f, 1 - a * 0.7f), Size * 1.2f);
	...
	else if(Group != GROUP_COLORTRAIL)
	{
		...
		while(i != -1)
		{
			...
			if(Group == GROUP_TRIANGLES || Group == GROUP_FLAMES)
			{
				...
				//	m_pClient->m_pEffects->Light(m_aParticles[i].m_Pos, 32*(1.0f-m_aParticles[i].m_Life /
				// m_aParticles[i].m_LifeSpan));
			}
			else
				Graphics()->SetColor(
					m_aParticles[i].m_Color.r, m_aParticles[i].m_Color.g, m_aParticles[i].m_Color.b, 1 - a);
	...
	else
	{
		...
		{
			float a = m_aParticles[i].m_Life / m_aParticles[i].m_LifeSpan;
			Graphics()->SetColor(m_aParticles[i].m_Color.x,
								 m_aParticles[i].m_Color.y,
								 m_aParticles[i].m_Color.z,
								 m_aParticles[i].m_Color.w * (1.0f - a));

			Out = vec2(m_aParticles[i].m_TrailDir.y, -m_aParticles[i].m_TrailDir.x) * m_aParticles[i].m_StartSize;

			IGraphics::CFreeformItem Freeform1(m_aParticles[i].m_StartPos.x - Out.x,
											   m_aParticles[i].m_StartPos.y - Out.y,
											   m_aParticles[i].m_StartPos.x + Out.x,
											   m_aParticles[i].m_StartPos.y + Out.y,
											   m_aParticles[i].m_EndPos.x - Out.x,
											   m_aParticles[i].m_EndPos.y - Out.y,
											   m_aParticles[i].m_EndPos.x + Out.x,
											   m_aParticles[i].m_EndPos.y + Out.y);
