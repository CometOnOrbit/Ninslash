	...
	*/
	...
	for(int x = x1; x <= x2; x++)
	{
		for(int y = y1; y <= y2; y++)
		{
			...
			{
				if(m_pBlockSyncTick[y * m_Width + x] > Client()->GameTick() ||
				   m_pBlockSyncTick[y * m_Width + x] < Client()->GameTick() - Client()->GameTickSpeed() * 6.0f)
				{
				...
				// fill
				{
					...
					if(m_pBlocks[y * m_Width + x] == 1)
						...
						Graphics()->QuadsSetSubsetFree(
							x1 + PixX, y0 + PixY, x2 - PixX, y0 + PixY, x1 + PixX, y1 - PixY, x2 - PixX, y1 - PixY);
					else if(m_pBlocks[y * m_Width + x] == 3)
						Graphics()->QuadsSetSubsetFree(
							x2 + PixX, y0 + PixY, x3 - PixX, y0 + PixY, x2 + PixX, y1 - PixY, x3 - PixX, y1 - PixY);
					// block 2
					else if(m_pBlocks[y * m_Width + x] == 4)
						Graphics()->QuadsSetSubsetFree(
							x0 + PixX, y1 + PixY, x1 - PixX, y1 + PixY, x0 + PixX, y2 - PixY, x1 - PixX, y2 - PixY);
					else if(m_pBlocks[y * m_Width + x] == 5)
						Graphics()->QuadsSetSubsetFree(
							x1 + PixX, y1 + PixY, x2 - PixX, y1 + PixY, x1 + PixX, y2 - PixY, x2 - PixX, y2 - PixY);
					else if(m_pBlocks[y * m_Width + x] == 6)
						Graphics()->QuadsSetSubsetFree(
							x2 + PixX, y1 + PixY, x3 - PixX, y1 + PixY, x2 + PixX, y2 - PixY, x3 - PixX, y2 - PixY);

					IGraphics::CFreeformItem FreeFormItem(
						p.x, p.y, p.x + Size, p.y, p.x, p.y + Size, p.x + Size, p.y + Size);
				...
				Graphics()->QuadsSetSubsetFree(bx0 + PixX,
											   0.25f + PixY,
											   bx1 - PixX,
											   0.25f + PixY,
											   bx0 + PixX,
				...
				if(!Left && !Top)
				{
					...
					IGraphics::CFreeformItem FreeFormItem(
						p.x + Size / 2, p.y, p.x, p.y, p.x + Size / 2, p.y + Size / 2, p.x, p.y + Size / 2);
				...
				if(!Right && !Top)
				{
					...
					IGraphics::CFreeformItem FreeFormItem(
						p.x, p.y, p.x + Size / 2, p.y, p.x, p.y + Size / 2, p.x + Size / 2, p.y + Size / 2);
				...
				if(!Left && !Bot)
				{
					...
					IGraphics::CFreeformItem FreeFormItem(
						p.x + Size / 2, p.y + Size / 2, p.x, p.y + Size / 2, p.x + Size / 2, p.y, p.x, p.y);
				...
				if(!Right && !Bot)
				{
					...
					IGraphics::CFreeformItem FreeFormItem(
						p.x, p.y + Size / 2, p.x + Size / 2, p.y + Size / 2, p.x, p.y, p.x + Size / 2, p.y);
				...
				Graphics()->QuadsSetSubsetFree(bx1 + PixX,
											   0.0f + PixY,
											   bx2 - PixX,
											   0.0f + PixY,
											   bx1 + PixX,
