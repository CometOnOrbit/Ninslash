...
void CPlayers::RenderHand(CTeeRenderInfo *pInfo, vec2 CenterPos, vec2 Dir, float AngleOffset, vec2 PostRotOffset)
{
	...
	// for (int i = 0; i < 2; i++)
	{
		...
		RenderTools()->SelectSprite(SPRITE_HAND1_4);
...
inline float AngularMixDirection(float Src, float Dst)
{
	return sinf(Dst - Src) > 0 ? 1 : -1;
}
inline float AngularDistance(float Src, float Dst)
{
	return asinf(sinf(Dst - Src));
}
						  ...
						  const CNetObj_PlayerInfo *pPlayerInfo)
{
	...
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_HOOK].m_Id);
		Graphics()->QuadsBegin();

		vec2 Pos = Position + vec2(0, -4);
		vec2 HookPos;

		if(Player.m_HookState == HOOK_GRABBEDBALL)
		{
			HookPos = mix(vec2(m_pClient->m_PredictedPrevBall.m_Pos.x, m_pClient->m_PredictedPrevBall.m_Pos.y),
						  vec2(m_pClient->m_PredictedBall.m_Pos.x, m_pClient->m_PredictedBall.m_Pos.y),
						  Client()->IntraGameTick());
		}
		else if(pPlayerChar->m_HookedPlayer != -1 && Player.m_HookState != HOOK_GRABBEDDROID)
		{
			if(m_pClient->m_Snap.m_pLocalInfo &&
			   pPlayerChar->m_HookedPlayer == m_pClient->m_Snap.m_pLocalInfo->m_ClientID)
			{
					...
					m_pClient->m_Snap.m_aCharacters[pPlayerChar->m_HookedPlayer].m_Active)
			{
				...
				HookPos = mix(vec2(pPrevChar->m_HookX, pPrevChar->m_HookY),
							  vec2(pPlayerChar->m_HookX, pPlayerChar->m_HookY),
							  Client()->IntraGameTick());
		}
		else
			HookPos = mix(vec2(Prev.m_HookX, Prev.m_HookY), vec2(Player.m_HookX, Player.m_HookY), IntraTick);

		Pos -= normalize(Pos - HookPos) * 12;
		...
		// render head
		Graphics()->QuadsSetSubset(1.0f / 4, 0, 1.0f / 4 + 0.5f, 1);
		...
		for(float f = 24; f < d && i < 1024; f += 24, i++)
		{
			...
			if(length(Dir * f) > d - 24)
			{
				l = 24 - (length(Dir * f) - (d - 24));
				p = HookPos + Dir * (f + (l - 24) / 2);
			}

			Array[i] = IGraphics::CQuadItem(p.x, p.y, l, 16);
		}
		...
		pCustomPlayerInfo->m_Hooking = true;
		pCustomPlayerInfo->m_HookDir = normalize(HookPos - Pos);
	}
	else
		pCustomPlayerInfo->m_Hooking = false;
}
							...
							const CNetObj_PlayerInfo *pPlayerInfo)
{
	...
	const bool IsCompactGun = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_COMPACT_GUN_HANDS);
	const bool IsTool = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_TOOL);
	const bool IsClaw = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_CLAW);
	const bool IsChainsaw = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_CHAINSAW);
	const bool IsFlamer = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_FLAMER);
	const bool IsSpinReflect = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_SPIN_REFLECT);
	const bool IsAreaShield = HasWeapon && WeaponHasBehavior(WeaponDefinition, WEAPON_BEHAVIOR_AREA_SHIELD);
	...
	vec2 Position = mix(vec2(Prev.m_X, Prev.m_Y), vec2(Player.m_X, Player.m_Y), IntraTick);
	vec2 Vel = mix(vec2(Prev.m_VelX / 256.0f, Prev.m_VelY / 256.0f),
				   vec2(Player.m_VelX / 256.0f, Player.m_VelY / 256.0f),
				   IntraTick);
	...
	*/
	...
	if(WeaponIsStatic)
	{
		...
		if(Player.m_AttackTick && pCustomPlayerInfo->m_RecoilTick < Player.m_AttackTick && !Paused)
		{
			...
			if(IsCompactGun)
			{
				...
				vec2 MuzzlePos =
					pCustomPlayerInfo->m_MuzzlePos + pCustomPlayerInfo->m_MuzzleDir * Moff.x + DirY * Moff.y;
	...
	if(WeaponCombat.m_FiringType == WFT_MELEE)
	{
		...
		if(pCustomPlayerInfo->m_MeleeTick < Player.m_AttackTick && !Paused)
		{
			...
			if(pCustomPlayerInfo->m_MeleeState == MELEE_UP)
			{
				...
				if(!IsTool && !IsClaw)
					m_pClient->m_pEffects->SwordHit(
						Position + vec2(0, -24) + Direction * 60, GetAngle(Direction), Flip, Charge);

				if(IsClaw)
					m_pClient->m_pEffects->ClawHit(
						Position + vec2(0, -24) + Direction * 50, GetAngle(Direction), Flip, Charge);
			}
			else
			{
				if(Direction.x < 0.0f)
			...
			m_pClient->AddFluidForce(Position + vec2(0, -24) + Direction * 80,
									 vec2(frandom() - frandom(), frandom() - frandom()) * 30);
			m_pClient->AddFluidForce(Position + vec2(0, -24) + Direction * 95,
									 vec2(frandom() - frandom(), frandom() - frandom()) * 30);
			...
			CustomStuff()->AddImpact(vec4(p.x - r, p.y - r, p.x + r, p.y + r),
									 CCustomStuff::IMPACT_HIT,
									 normalize(p - Position + vec2(0, -0.3f)));
		}
	}
	// spinning weapons / scythe
	else if(IsSpinReflect)
	{
		// melee attack effect
		if(pCustomPlayerInfo->m_MeleeTick < Player.m_AttackTick && !Paused)
		{
	...
	if(!InAir && WantOtherDir && length(Vel * 50) > 350.0f) // from 500 to 350
	{
		...
		m_pClient->m_pEffects->SkidTrail(Position + vec2(-Player.m_Direction * 6, 12),
	...
	// draw aim line
	if(!CustomStuff()->m_Inventory && pPlayerInfo->m_Local && WeaponCombat.m_Aimline &&
	   pCustomPlayerInfo->m_EffectIntensity[EFFECT_SPAWNING] < 0.9f)
	{
		...
		IGraphics::CFreeformItem Freeform2(From.x - Out.x,
										   From.y - Out.y,
										   From.x + Out.x,
										   From.y + Out.y,
										   Pos.x - Out.x,
	...
	if(!Paused && IsChainsaw && Player.m_AttackTick > Client()->GameTick() - 500 * Client()->GameTickSpeed() / 1000)
	{
		...
		vec2 p = Position + Direction * g_pData->m_Weapons.m_aId[WEAPON_CHAINSAW].m_Offsetx +
				 CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID].m_WeaponRecoil +
				 CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID].m_Weapon2Recoil + WeaponOffset;
	...
	if(HasWeapon && WeaponVisual.m_RenderType == WRT_WEAPON1)
	{
		...
		const int CustomHeldTexture = g_WeaponResources.HeldTexture(Player.m_WeaponSpec);
		...
		// RenderTools()->SelectSprite(g_pData->m_Weapons.m_aId[iw].m_pSpriteBody, Direction.x < 0 ? SPRITE_FLAG_FLIP_Y
		// : 0);
...

		{
			...
			if(a < 1)
			{
				...
				if(CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID].m_WeaponRecoilLoaded)
				{
					...
					if(IsChainsaw)
					{
						...
						pCustomPlayerInfo->m_LastChainsawSoundTick =
							Client()->GameTick() + 500 * Client()->GameTickSpeed() / 1000;
			...
			p = Position + Dir * Offset.x + CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID].m_WeaponRecoil +
				CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID].m_Weapon2Recoil + WeaponOffset;

			// chainsaw shaking
			if(!Paused && IsChainsaw &&
			   Player.m_AttackTick > Client()->GameTick() - 500 * Client()->GameTickSpeed() / 1000)
			{
				...
				if(WeaponCombat.m_FiringType != WFT_HOLD)
				{
					...
					for(int i = 0; i < 4; i++)
					{
						if(pCustomPlayerInfo->m_aMuzzleWeapon[i])
						{
							...
							RenderTools()->SelectSprite(SPRITE_MUZZLE1_1 + pCustomPlayerInfo->m_aMuzzleType[i] * 4 +
															pCustomPlayerInfo->m_aMuzzleTime[i] * 4,
														SPRITE_FLAG_FLIP_X);
							...
							m_pClient->m_pEffects->SimpleLight(MuzzlePos, vec4(1.0f, 1.0f, 1.0f, 0.5f), 50);
						}
					}
				}
				else
				...
				if (WeaponCombat.m_FiringType != WFT_HOLD)
				{
					...
					for (int i = 0; i < 4; i++)
					{
						if (pCustomPlayerInfo->m_aMuzzleWeapon[i])
						{
							...
							RenderTools()->SelectSprite(SPRITE_MUZZLE1_1 + pCustomPlayerInfo->m_aMuzzleType[i]*4 +
				pCustomPlayerInfo->m_aMuzzleTime[i]*4, SPRITE_FLAG_FLIP_X);
		...
		if(IsFlamer)
		{
			...
			if(pCustomPlayerInfo->m_FlameState > 0)
			{
				...
				for(int i = 0; i <= Slices; i++)
				{
					...
					{
						Flames++;
						m_pClient->m_pEffects->Flame(PrevPos +
														 vec2(frandom() - frandom(), frandom() - frandom()) * 12.0f,
													 vec2(frandom() - frandom(), frandom() - frandom()) * 200.0f,
													 0.5f,
													 true);
					}

					bool NoCol = true;
					if(Collision()->IntersectLine(StartPos, p1, 0x0, &p1))
						NoCol = false;
					if(Collision()->IntersectLine(StartPos, p3, 0x0, &p3))
						NoCol = false;
					if(Collision()->IntersectLine(StartPos, p2, 0x0, &p2))
						NoCol = false;
					if(Collision()->IntersectLine(StartPos, p4, 0x0, &p4))
						NoCol = false;
				...
				// m_pClient->m_pEffects->BoxLight((LightStartPos+FinalPos)/2-DirY*OffsetY*(Flip?1:0), vec4(1.0f, 0.75f,
				// 0.5f, 0.4f), vec2(80, length(LightStartPos-FinalPos)+20), atan2(Dir.y, Dir.x)+pi/2);
		...
		const int CustomMuzzleTexture = g_WeaponResources.MuzzleTexture(Player.m_WeaponSpec);
		const int MuzzleAge = Client()->GameTick() - Player.m_AttackTick;
		if(CustomMuzzleTexture >= 0 && MuzzleAge >= 0 && MuzzleAge < 5)
		{
			const float Alpha = 1.0f - (MuzzleAge + IntraTick) / 5.0f;
			const vec2 MuzzleOffset = WeaponVisual.m_MuzzleOffset;
			const vec2 DirY(-Dir.y, Dir.x);
			const vec2 MuzzlePos =
				p + Dir * MuzzleOffset.x + DirY * (Direction.x < 0 ? -MuzzleOffset.y : MuzzleOffset.y);
			Graphics()->TextureSet(CustomMuzzleTexture);
			Graphics()->QuadsBegin();
			Graphics()->QuadsSetRotation(Angle);
			Graphics()->QuadsSetSubset(0, 0, 1, 1);
			Graphics()->SetColor(1, 1, 1, clamp(Alpha, 0.0f, 1.0f));
			IGraphics::CQuadItem MuzzleQuad(MuzzlePos.x, MuzzlePos.y, 48.0f, 32.0f);
	...
	if(Player.m_Jetpack == 1 && Player.m_JetpackPower > 0 && Player.m_Wallrun == 0)
	{
		...
		*/
		...
		if(!Paused && pCustomPlayerInfo->m_LastJetpackSoundTick <= Client()->GameTick())
		{
			...
			m_pClient->m_pSounds->PlayAt(
				CSounds::CHN_WORLD, SOUND_JETPACK1 + pCustomPlayerInfo->m_LastJetpackSound, 1.0f, Position);
	...
	if(IsSpinReflect && !Paused)
	{
		...
		if(pCustomPlayerInfo->MeleeImpact() > 0)
		{
			...
			CustomStuff()->AddImpact(
				vec4(p.x - r * 0.8f, p.y, p.x + r * 0.8f, p.y + r), CCustomStuff::IMPACT_SCYTHE, vec2(-0.4f, -1.0f));
		}
	...
	*/
	...
	if(WantedAnimation == PANIM_JUMP)
		pCustomPlayerInfo->UnlockFeet(Vel);
	else
		pCustomPlayerInfo->m_AreFeetLocked = true;
	...
	RenderTools()->RenderPlayer(
		&CustomStuff()->m_aPlayerInfo[pInfo.m_ClientID], &RenderInfo, Player.m_Emote, Direction, Position);
	...
	if(Player.m_HandJetpack)
	{
		...
		if(!Paused && pCustomPlayerInfo->m_LastTurboSoundTick <= Client()->GameTick())
		{
			...
			m_pClient->m_pSounds->PlayAt(
				CSounds::CHN_WORLD, SOUND_TURBO1 + pCustomPlayerInfo->m_LastTurboSound, 1.0f, Position);
	...
	*/
	...
	if(Phase1Tick > 0 && pCustomPlayerInfo->WeaponChargeProgress() > 0.0f && IsAreaShield)
	{
	...
	if(pCustomPlayerInfo->m_EffectIntensity[EFFECT_ELECTRODAMAGE] > 0.5f && Client()->GameTick() % 5 == 1)
		m_pClient->m_pEffects->Electrospark(Position + vec2((frandom() - frandom()) * 16.0f, -frandom() * 48.0f),
											30.0f);
...
void CPlayers::OnRender()
{
	...
	for(int p = 0; p < 4; p++)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(pInfo)
			{
				const CNetObj_PlayerInfo *pPrevPlayerInfo =
					pPrevInfo ? (const CNetObj_PlayerInfo *)pPrevInfo : (const CNetObj_PlayerInfo *)pInfo;
				//
				...
				if((p < 2 && pCustomPlayerInfo->BackHook()) || (p >= 2 && !pCustomPlayerInfo->BackHook()))
					RenderHook(&PrevChar, &CurChar, pPrevPlayerInfo, (const CNetObj_PlayerInfo *)pInfo);
				else
	...
	for(int i = 0; i < MAX_CLIENTS; i++)
	{
		...
		if(pInfo)
		{
			...
			if(Local)
			{
				...
				const int aDefinitionIds[] = {pPlayerInfo->m_Weapon1DefinitionId,
											  pPlayerInfo->m_Weapon2DefinitionId,
											  pPlayerInfo->m_Weapon3DefinitionId,
											  pPlayerInfo->m_Weapon4DefinitionId};
				const int aLevels[] = {pPlayerInfo->m_Weapon1Level,
									   pPlayerInfo->m_Weapon2Level,
									   pPlayerInfo->m_Weapon3Level,
									   pPlayerInfo->m_Weapon4Level};
				for(int Slot = 0; Slot < 4; ++Slot)
					if(!CWeaponCatalog::TryFromProtocol(
						   aDefinitionIds[Slot], aLevels[Slot], &CustomStuff()->m_aSnapWeapon[Slot]))
