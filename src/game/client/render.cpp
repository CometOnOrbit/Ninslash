...
static mat33 CalcTransformationMatrix(vec2 Trans, vec2 Scale, float Rotation)
{
	...
	return mat33(Scale.x * c, -Scale.x * s, Trans.x, Scale.y * s, Scale.y * c, Trans.y, 0.0f, 0.0f, 1.0f);
}
...
void CAnimSkeletonInfo::UpdateBones(float Time,
									CSpineAnimation *pAnimation,
									CSkeletonAnimation *pAnimData,
									int WeaponAngle,
									CDroidAnim *pDroidAnim,
									CSpineAnimation *pBaseAnimation,
									float BaseTime)
{
	...
	for(int i = 0; i < m_lBones.size(); i++)
	{
		...
		auto ApplyAnimation = [&](CSpineAnimation *pSource, float SourceTime)
		{
			...
			if(pBoneTimeline)
			{
				if(pBoneTimeline->m_lTranslations.size())
					CRenderTools::RenderEvalSkeletonAnim<CSpineBoneKeyframeTranslate>(
						pBoneTimeline->m_lTranslations.base_ptr(),
						pBoneTimeline->m_lTranslations.size(),
						SourceTime,
						(float *)&Position);
				if(pBoneTimeline->m_lScales.size())
					CRenderTools::RenderEvalSkeletonAnim<CSpineBoneKeyframeScale>(pBoneTimeline->m_lScales.base_ptr(),
																				  pBoneTimeline->m_lScales.size(),
																				  SourceTime,
																				  (float *)&Scale);
				if(pBoneTimeline->m_lRotations.size())
					CRenderTools::RenderEvalSkeletonAnim<CSpineBoneKeyframeRotate>(
						pBoneTimeline->m_lRotations.base_ptr(),
						pBoneTimeline->m_lRotations.size(),
						SourceTime,
						(float *)&Rotation);
			}
		};
		...
		if(pAnimation || pBaseAnimation)
		{
			...
			if(pDroidAnim)
			{
				...
				else if(pBone->m_SpecialType == BST_WEAPON0 || pBone->m_SpecialType == BST_WEAPON1 ||
						pBone->m_SpecialType == BST_WEAPON2)
					Rotation = pDroidAnim->m_aValue[CDroidAnim::TURRET_ANGLE] * pi / 180.0f -
							   pDroidAnim->m_aValue[CDroidAnim::BODY_ANGLE];
			}
			else
				// walkers
				if(pBone->m_SpecialType == BST_WEAPON0 || pBone->m_SpecialType == BST_WEAPON1 ||
				   pBone->m_SpecialType == BST_WEAPON2)
				{
					Rotation = WeaponAngle * pi / 180.0f;
				}
			...
			if(pAnimData)
			{
				if(pAnimData->m_Anim != PANIM_IDLE &&
				   (pBone->m_SpecialType == BST_FFOOT || pBone->m_SpecialType == BST_BFOOT))
				{
				...
				if(pBone->m_SpecialType == BST_HEAD)
				{
					...
					pAnimData->m_HeadTargetAngle =
						(pAnimData->m_HeadTilt + pAnimData->m_HeadTiltCorrect) * 0.5f; // - pBone->m_Rotation;
										 ...
										 const std::map<string, CSpineAnimation> &mAnimations)
{
	...
	for(SkinMap::const_iterator IterSkin = mSkins.begin(); IterSkin != mSkins.end(); IterSkin++)
	{
		...
		for(std::map<string, std::map<string, CSpineAttachment>>::const_iterator IterSlot = IterSkin->second.begin();
			IterSlot != IterSkin->second.end();
			IterSlot++)
		{
			string SlotName = IterSlot->first;

			for(std::map<string, CSpineAttachment>::const_iterator IterAttachment = IterSlot->second.begin();
				IterAttachment != IterSlot->second.end();
				IterAttachment++)
			{
				...
				switch(Attachment.m_Type)
				{
					...
					case SPINE_ATTACHMENT_SKINNED_MESH:
					{
						...
						{
							pAnimAttachment->m_aUVs.add(
								vec2(Attachment.m_SkinnedMesh.m_aUV[uv], Attachment.m_SkinnedMesh.m_aUV[uv + 1]));
						}
						...
						for(int v = 0; v < Attachment.m_SkinnedMesh.m_aVertices.size();)
						{
							...
							for(int vb = 0; vb < NumBones; vb++, v += 4)
							{
								...
								Vertex.m_aPoints.add(vec2(Attachment.m_SkinnedMesh.m_aVertices[v + 1],
														  -Attachment.m_SkinnedMesh.m_aVertices[v + 2]));
...
bool CRenderTools::LoadAtlasFromSpine(CTextureAtlas *pAtlas, const CSpineAtlas &rSpineAtlas)
{
	...
	for(int i = 0; i < rSpineAtlas.m_lPages.size(); i++)
	{
		...
		Page.m_TexId = Graphics()->LoadTextureRaw(
			Page.m_Width, Page.m_Height, Page.m_Format, Page.m_pData, CImageInfo::FORMAT_AUTO, 0);
...
void CRenderTools::DrawRoundRectExt(float x, float y, float w, float h, float r, int Corners)
{
	...
	for(int i = 0; i < Num; i += 2)
	{
		...
		if(Corners & 1) // TL
			ArrayF[NumItems++] = IGraphics::CFreeformItem(x + r,
														  y + r,
														  x + (1 - Ca1) * r,
														  y + (1 - Sa1) * r,
														  x + (1 - Ca3) * r,
		...
		if(Corners & 2) // TR
			ArrayF[NumItems++] = IGraphics::CFreeformItem(x + w - r,
														  y + r,
														  x + w - r + Ca1 * r,
														  y + (1 - Sa1) * r,
														  x + w - r + Ca3 * r,
		...
		for(int i = 0; i < NumSlots; i++)
		{
			...
			switch(pAttachmentBase->m_Type)
			{
				case ATTACHMENT_SPRITE:
				{
					...
					if(pAttachment->m_SpecialType != AST_HEAD && pAttachment->m_SpecialType != AST_HEAD_ASSET &&
					   pAttachment->m_SpecialType != AST_HAT && pAttachment->m_SpecialType != AST_EYES)
						continue;
					...
					mat33 AttachmentParent = CalcTransformationMatrix(
						pAttachment->m_Position, pAttachment->m_Scale, pAttachment->m_Rotation);

					vec3 p0, p1, p2, p3;
					p0 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(-pAttachment->m_Width / 2.0f, -pAttachment->m_Height / 2.0f, 1.0f);
					p1 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(pAttachment->m_Width / 2.0f, -pAttachment->m_Height / 2.0f, 1.0f);
					p2 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(-pAttachment->m_Width / 2.0f, pAttachment->m_Height / 2.0f, 1.0f);
					p3 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(pAttachment->m_Width / 2.0f, pAttachment->m_Height / 2.0f, 1.0f);
					...
					else
					{
						...
						t3 = vec2((pSprite->m_X + pSprite->m_Width) / pPage->m_Width,
								  (pSprite->m_Y + pSprite->m_Height) / pPage->m_Height);
					...
					IGraphics::CFreeformItem FreeFormItem(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
					...
					if(SubsetType == 3 || SubsetType == 4)
					{
						...
						IGraphics::CFreeformItem FreeFormItem2(p0.x, p0.y, p1.x, p1.y, p2.x, p2.y, p3.x, p3.y);
		...
		for(int i = 0; i < NumSlots; i++)
		{
			...
			switch(pAttachmentBase->m_Type)
			{
				case ATTACHMENT_SPRITE:
				{
					...
					if(strlen(pSlot->m_Name) > 3 && pSlot->m_Name[0] == '_' && pSlot->m_Name[1] == 's' &&
					   pSlot->m_Name[2] == 'c')
					{
						// Graphics()->ShaderBegin(SHADER_SPAWN, 0.05f);
						Screen = true;
					}
					else if(strlen(pSlot->m_Name) > 3 && pSlot->m_Name[0] == 's' && pSlot->m_Name[1] == 'c' &&
							pSlot->m_Name[2] == 'r')
					{
					...
					if(Screen)
					{
						...
						{
							case 0:
								Graphics()->SetColor(0.5f, 0.7f, 1.0f, 1);
								break;
							case 1:
								Graphics()->SetColor(0.4f, 1.0f, 0.4f, 1);
								break;
							case 2:
								Graphics()->SetColor(1.0f, 0.5f, 0.5f, 1);
								break;
							case 3:
								Graphics()->SetColor(0.2f, 1.0f, 1.0f, 1);
								break;
...
void CRenderTools::RenderWalker(vec2 Pos, int Anim, float Time, int Dir, float Angle, int Status, int Type)
{
	...
	pSkeleton->UpdateBones(Time, pAnimation, 0, Angle);
...
void CRenderTools::RenderStarDroid(
	vec2 Pos, int Anim, float Time, int Dir, float Angle, int Status, CDroidAnim *pDroidAnim)
{
	vec2 Position = Pos;
	...
	pSkeleton->UpdateBones(Time, pAnimation, 0, Angle, pDroidAnim);
		...
		for(int i = 0; i < NumSlots; i++)
		{
			...
			switch(pAttachmentBase->m_Type)
			{
				case ATTACHMENT_SPRITE:
				{
					...
					if(pAttachment->m_SpecialType == AST_JET1 || pAttachment->m_SpecialType == AST_JET2 ||
					   pAttachment->m_SpecialType == AST_JET)
					{
						if(Status != DROIDSTATUS_TERMINATED)
						{
							...
							if(pAttachment->m_SpecialType == AST_JET1)
							{
								...
								pDroidAnim->m_aVectorValue[CDroidAnim::THRUST1_VEL] =
									normalize(vec2(p1.x - p0.x, p1.y - p0.y));
							}
							else if(pAttachment->m_SpecialType == AST_JET2)
							{
								pDroidAnim->m_aVectorValue[CDroidAnim::THRUST2_POS] = vec2(p.x, p.y);
								pDroidAnim->m_aVectorValue[CDroidAnim::THRUST2_VEL] =
									normalize(vec2(p1.x - p0.x, p1.y - p0.y));
							}
						}
					}
...
void CRenderTools::RenderCrawlerDroid(
	vec2 Pos, int Anim, float Time, int Dir, float Angle, int Status, CDroidAnim *pDroidAnim, bool Render)
{
	vec2 Position = Pos;
				...
				case ATTACHMENT_SPRITE:
				{
					...
					if(Render)
					{
						...
						else
						{
							vec2 t0, t1, t2, t3;
							t0 = vec2(pSprite->m_X / pPage->m_Width, pSprite->m_Y / pPage->m_Height);
							t1 = vec2((pSprite->m_X + pSprite->m_Width) / pPage->m_Width,
									  pSprite->m_Y / pPage->m_Height);
							t2 = vec2(pSprite->m_X / pPage->m_Width,
									  (pSprite->m_Y + pSprite->m_Height) / pPage->m_Height);
							t3 = vec2((pSprite->m_X + pSprite->m_Width) / pPage->m_Width,
									  (pSprite->m_Y + pSprite->m_Height) / pPage->m_Height);
...
void CRenderTools::RenderCrawlerLegs(CDroidAnim *pDroidAnim)
{
	...
	if(abs(pDroidAnim->m_aVectorValue[CDroidAnim::ATTACH1_POS].x - pDroidAnim->m_aLegPos[0].x) > 300 ||
	   abs(pDroidAnim->m_aVectorValue[CDroidAnim::ATTACH1_POS].y - pDroidAnim->m_aLegPos[0].y) > 300)
		return;
...
void CRenderTools::RenderArm(CPlayerInfo *PlayerInfo, CTeeRenderInfo *pInfo, vec2 Pos, vec2 PlayerPos)
{
	...
	{
		if(pInfo->m_Body == 6)
			Graphics()->SetColor(pInfo->m_ColorBody.r / 2.5f,
								 pInfo->m_ColorBody.g / 2.5f,
								 pInfo->m_ColorBody.b / 2.5f,
								 pInfo->m_ColorBody.a);
		else
			Graphics()->SetColor(pInfo->m_ColorBody.r / 1.5f,
								 pInfo->m_ColorBody.g / 1.5f,
								 pInfo->m_ColorBody.b / 1.5f,
								 pInfo->m_ColorBody.a);
...
void CRenderTools::RenderMelee(CPlayerInfo *PlayerInfo, CTeeRenderInfo *pInfo, vec2 Dir, vec2 Pos)
{
	if(PlayerInfo->WeaponRenderType() == WRT_SPIN)
	{
		...
		if(abs(Speed) > 0.3f)
		{
			...
			RenderWeapon(PlayerInfo->m_Weapon,
						 p,
						 vec2(cos(WAngle2), sin(WAngle2)) * WeaponDir,
						 WEAPON_GAME_SIZE,
						 true,
						 Flags,
						 0.25f);
		...
		RenderWeapon(
			PlayerInfo->m_Weapon, p, vec2(cos(WAngle), sin(WAngle)) * WeaponDir, WEAPON_GAME_SIZE, true, Flags, Alpha2);
...

		{
			...
			IGraphics::CQuadItem QuadItem(WeaponPos.x + Offset.x, WeaponPos.y + Offset.y,
		Size.x*PlayerInfo->MeleeSize(), Size.y*PlayerInfo->MeleeSize()); Graphics()->QuadsDraw(&QuadItem, 1);
		}
		...
		*/
		...
		Graphics()->QuadsSetRotation(WeaponAngle - pi / 2);
		{
			...
			IGraphics::CQuadItem QuadItem(
				WeaponPos.x + Offset.x, WeaponPos.y + Offset.y, 2 * HandBaseSize, 2 * HandBaseSize);
			Graphics()->QuadsDraw(&QuadItem, 1);
		}
	...
	else if(PlayerInfo->WeaponRenderType() == WRT_MELEE)
	{
		...
		RenderWeapon(PlayerInfo->m_Weapon,
					 p + HandPos,
					 vec2(cos(WAngle), sin(WAngle)) * WeaponDir,
					 WEAPON_GAME_SIZE,
					 true,
					 Flags,
					 Alpha2);
...
{
	float Visibility = max(pCustomPlayerInfo->m_EffectIntensity[EFFECT_SPAWNING],
						   pCustomPlayerInfo->m_EffectIntensity[EFFECT_INVISIBILITY]);
...
void CRenderTools::SetShadersForWeapon(CPlayerInfo *pCustomPlayerInfo)
{
	...
	// dont flash shuriken
	if(WeaponHasBehavior(Profile.m_Definition, WEAPON_BEHAVIOR_SHURIKEN))
		ChargeLevel = 0;

	SetShadersForWeapon(pCustomPlayerInfo->m_Weapon, ChargeLevel, 1.0f - Visibility, Electro, Damage, Deathray);
}

void CRenderTools::SetShadersForWeapon(
	const CWeaponSpec &Weapon, float Charge, float Visibility, float Electro, float Damage, float Deathray)
{
	CResolvedWeaponProfile Profile;
	if(!CWeaponCatalog::TryResolve(Weapon, &Profile))
		return;
	if(WeaponHasBehavior(Profile.m_Definition, WEAPON_BEHAVIOR_CLUSTER) &&
	   Weapon.m_Level == WEAPON_CLUSTER_FRAGMENT_LEVEL)
	{
		CWeaponSpec Base = Weapon;
		Base.m_Level = 0;
		CWeaponCatalog::TryResolve(Base, &Profile);
	}
...
void CRenderTools::RenderForegroundHand(CPlayerInfo *PlayerInfo)
{
	...
	Graphics()->QuadsBegin();
	Graphics()->SetColor(PlayerInfo->m_RenderInfo.m_ColorSkin.r,
						 PlayerInfo->m_RenderInfo.m_ColorSkin.g,
						 PlayerInfo->m_RenderInfo.m_ColorSkin.b,
						 PlayerInfo->m_RenderInfo.m_ColorSkin.a);
...
void CRenderTools::RenderFreeHand(
	CPlayerInfo *PlayerInfo, CTeeRenderInfo *pInfo, int Hand, vec2 Dir, vec2 Pos, bool Behind)
{
	bool FlipY = false;
	...
	if(Hand == HAND_WEAPON &&
	   (PlayerInfo->WeaponRenderType() == WRT_SPIN || PlayerInfo->WeaponRenderType() == WRT_MELEESMALL ||
		PlayerInfo->WeaponRenderType() == WRT_MELEE || PlayerInfo->WeaponRenderType() == WRT_ITEM1 ||
		PlayerInfo->WeaponRenderType() == WRT_WEAPON2))
	{
		vec2 DirX = d;
		vec2 DirY(-d.y, d.x);
		...
		if(CWeaponCatalog::TryResolve(PlayerInfo->m_Weapon, &WeaponProfile) &&
		   WeaponHasBehavior(WeaponProfile.m_Definition, WEAPON_BEHAVIOR_SHURIKEN))
			wd = GetDirection(GetAngle(d) + PlayerInfo->m_SpinningAngle *
												(d.x > 0.0f ? -1 : 1)); //  * PlayerInfo->WeaponChargeProgress()
...
void CRenderTools::RenderWeapon(const CWeaponSpec &Weapon,
								vec2 Pos,
								vec2 Dir,
								float Size,
								bool BeginQuads,
								...
								float OverallAlpha)
{
	...
	if(KillMessage && WeaponHasBehavior(Definition, WEAPON_BEHAVIOR_TOOL))
		return;

	ivec2 WSize = Profile.m_Visual.m_VisualSize;
	const int CustomTexture = g_WeaponResources.HeldTexture(Weapon);
	if(CustomTexture >= 0)
	{
		if(BeginQuads)
		{
			Graphics()->TextureSet(CustomTexture);
			Graphics()->QuadsBegin();
			Graphics()->QuadsSetRotation(GetAngle(Dir));
			Graphics()->SetColor(1, 1, 1, OverallAlpha);
		}
		const int EffectiveFlags = NoFlags ? Flags : ((Dir.x < 0 ? SPRITE_FLAG_FLIP_Y : 0) ^ Flags);
		if(EffectiveFlags & SPRITE_FLAG_FLIP_Y)
			Graphics()->QuadsSetSubset(0, 1, 1, 0);
		else
			Graphics()->QuadsSetSubset(0, 0, 1, 1);
		IGraphics::CQuadItem QuadItem(Pos.x, Pos.y, int(Size) * WSize.x, int(Size) * WSize.y);
		Graphics()->QuadsDraw(&QuadItem, 1);
		if(BeginQuads)
			Graphics()->QuadsEnd();
		return;
	...
	if(Profile.m_Visual.m_RenderType == WRT_SPIN || Profile.m_Visual.m_RenderType == WRT_MELEE)
	{
		...
		// back blade
		if(WeaponHasBehavior(Definition, WEAPON_BEHAVIOR_SPIN_REFLECT))
		{
			...
			IGraphics::CQuadItem QuadItem2(
				Pos.x - Dir.x * Size * 7 / 2, Pos.y - Dir.y * Size * 7 / 2, Size * WSize2.x, Size * WSize2.y);
			Graphics()->QuadsDraw(&QuadItem2, 1);
			Graphics()->QuadsSetRotation(GetAngle(Dir));
		}
	...
	else
	{
		...
		if(Part1SpriteIndex >= 0)
		{
			SelectSprite(SPRITE_WEAPON_PART1_BG_0 + Part1SpriteIndex,
						 NoFlags ? Flags : (Dir.x < 0 ? SPRITE_FLAG_FLIP_Y : 0));
			// DrawSprite(Pos.x, Pos.y, Size);
			IGraphics::CQuadItem QuadItem(Pos.x, Pos.y, Size * WSize.x, Size * WSize.y);
			Graphics()->QuadsDraw(&QuadItem, 1);
		}
		...
		if(Part2SpriteIndex >= 0)
		{
			SelectSprite(SPRITE_WEAPON_PART2_0 + Part2SpriteIndex,
						 NoFlags ? Flags : (Dir.x < 0 ? SPRITE_FLAG_FLIP_Y : 0));
			IGraphics::CQuadItem QuadItem2(Pos.x + Dir.x * Size * (WSize.x - 1),
										   Pos.y + Dir.y * Size * (WSize.x - 1),
										   Size * WSize2.x,
										   Size * WSize2.y);
			Graphics()->QuadsDraw(&QuadItem2, 1);
		}
		...
		if(Part1SpriteIndex >= 0)
		{
			SelectSprite(SPRITE_WEAPON_PART1_0 + Part1SpriteIndex,
						 NoFlags ? Flags : (Dir.x < 0 ? SPRITE_FLAG_FLIP_Y : 0));
			// DrawSprite(Pos.x, Pos.y, Size);
			IGraphics::CQuadItem QuadItem3(Pos.x, Pos.y, Size * WSize.x, Size * WSize.y);
			Graphics()->QuadsDraw(&QuadItem3, 1);
...
void CRenderTools::RenderSkeleton(vec2 Position,
								  const CTeeRenderInfo *pInfo,
								  CSkeletonAnimation *AnimData,
								  float Rotation,
								  CAnimSkeletonInfo *pSkeleton,
								  CTextureAtlas *pAtlas,
								  CPlayerInfo *PlayerInfo)
{
		...
		for(int i = 0; i < NumSlots; i++)
		{
			...
			switch(pAttachmentBase->m_Type)
			{
				case ATTACHMENT_SPRITE:
				{
					...
					vec3 p0, p1, p2, p3;
					p0 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(-pAttachment->m_Width / 2.0f, -pAttachment->m_Height / 2.0f * HeightScale, 1.0f);
					p1 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(pAttachment->m_Width / 2.0f, -pAttachment->m_Height / 2.0f * HeightScale, 1.0f);
					p2 = TransformationWorld * pBone->m_Transform * AttachmentParent *
						 vec3(-pAttachment->m_Width / 2.0f, pAttachment->m_Height / 2.0f * HeightScale, 1.0f);
					...
					if(((PlayerInfo && PlayerInfo->m_Jetpack) || AnimData->GetAnimation() == PANIM_SLIDEKICK) &&
					   pAttachment->m_SpecialType == AST_FOOT)
					{
					...
					if(pAttachment->m_SpecialType == AST_BODY)
					{
						...
						if(PlayerInfo && PlayerInfo->m_BombCarrier)
						{
							...
							Graphics()->QuadsSetRotation(GetAngle(di) + pi / 2);
							{
								float s = 32;
								IGraphics::CQuadItem QuadItem(
									p.x + di.x * s, p.y + di.y * s, s * 2 * (AnimData->m_Flip ? -1 : 1), s * 2);
								Graphics()->QuadsDraw(&QuadItem, 1);
							}
					...
					else if(pAttachment->m_SpecialType == AST_HAND)
					{
						if(PlayerInfo)
						{
							// PlayerInfo->SetHandTarget(HAND_FREE, (p0+p1+p2+p3) / 4.0f - vec3(Position.x, Position.y,
							// 0)); PlayerInfo->SetHandTarget(HAND_WEAPON, vec3(0, -4, 0) + (p0+p1+p2+p3) / 4.0f -
					...
					if (PlayerInfo)
					{
						if (pAttachment->m_SpecialType == AST_SPLATTER1)
							Graphics()->SetColor(PlayerInfo->m_aSplatterColor[0].r, PlayerInfo->m_aSplatterColor[0].g,
					PlayerInfo->m_aSplatterColor[0].b, PlayerInfo->m_aSplatter[0]); if (pAttachment->m_SpecialType ==
					AST_SPLATTER2) Graphics()->SetColor(PlayerInfo->m_aSplatterColor[1].r,
					PlayerInfo->m_aSplatterColor[1].g, PlayerInfo->m_aSplatterColor[1].b, PlayerInfo->m_aSplatter[1]);
						if (pAttachment->m_SpecialType == AST_SPLATTER3)
							Graphics()->SetColor(PlayerInfo->m_aSplatterColor[2].r, PlayerInfo->m_aSplatterColor[2].g,
					PlayerInfo->m_aSplatterColor[2].b, PlayerInfo->m_aSplatter[2]); if (pAttachment->m_SpecialType ==
					AST_SPLATTER4) Graphics()->SetColor(PlayerInfo->m_aSplatterColor[3].r,
					PlayerInfo->m_aSplatterColor[3].g, PlayerInfo->m_aSplatterColor[3].b, PlayerInfo->m_aSplatter[3]);
						if (pAttachment->m_SpecialType == AST_SPLATTER5)
							Graphics()->SetColor(PlayerInfo->m_aSplatterColor[4].r, PlayerInfo->m_aSplatterColor[4].g,
					PlayerInfo->m_aSplatterColor[4].b, PlayerInfo->m_aSplatter[4]); if (pAttachment->m_SpecialType ==
					AST_SPLATTER6) Graphics()->SetColor(PlayerInfo->m_aSplatterColor[5].r,
					PlayerInfo->m_aSplatterColor[5].g, PlayerInfo->m_aSplatterColor[5].b, PlayerInfo->m_aSplatter[5]);
						if (pAttachment->m_SpecialType == AST_SPLATTER7)
							Graphics()->SetColor(PlayerInfo->m_aSplatterColor[6].r, PlayerInfo->m_aSplatterColor[6].g,
					PlayerInfo->m_aSplatterColor[6].b, PlayerInfo->m_aSplatter[6]); if (pAttachment->m_SpecialType ==
					AST_SPLATTER8) Graphics()->SetColor(PlayerInfo->m_aSplatterColor[7].r,
					PlayerInfo->m_aSplatterColor[7].g, PlayerInfo->m_aSplatterColor[7].b, PlayerInfo->m_aSplatter[7]);
					}
					*/
				...
				case ATTACHMENT_SKINNED_MESH:
				{
					...
					for(int v = 0; v < pAttachment->m_aVertices.size(); v++)
					{
						...
						for(int b = 0; b < rVertex.m_aBones.size(); b++)
						{
							...
							vec3 vp =
								pLocalBone->m_Transform * vec3(rVertex.m_aPoints[b].x, rVertex.m_aPoints[b].y, 1.0f);
							p.x += vp.x * weight;
							p.y += vp.y * weight;
						}
					...
					for(int v = 0; v < pAttachment->m_aUVs.size(); v++)
					{
						...
						if(!pSprite->m_Rotate)
						{
							uv = vec2((pSprite->m_X + pSprite->m_Width * pAttachment->m_aUVs[v].x) / pPage->m_Width,
									  (pSprite->m_Y + pSprite->m_Height * pAttachment->m_aUVs[v].y) / pPage->m_Height);
						}
						else
						{
							uv =
								vec2((pSprite->m_X + pSprite->m_Width * pAttachment->m_aUVs[v].y) / pPage->m_Width,
									 (pSprite->m_Y + pSprite->m_Height - pSprite->m_Height * pAttachment->m_aUVs[v].x) /
										 pPage->m_Height);
						}
					...
					{
						IGraphics::CTriangleItem TriangleItem(aVerticesWorld[pAttachment->m_aTriangles[i]].x,
															  aVerticesWorld[pAttachment->m_aTriangles[i]].y,
															  aVerticesWorld[pAttachment->m_aTriangles[i + 1]].x,
															  aVerticesWorld[pAttachment->m_aTriangles[i + 1]].y,
															  aVerticesWorld[pAttachment->m_aTriangles[i + 2]].x,
															  aVerticesWorld[pAttachment->m_aTriangles[i + 2]].y);

						Graphics()->QuadsSetSubsetFree(aUVs[pAttachment->m_aTriangles[i]].x,
													   aUVs[pAttachment->m_aTriangles[i]].y,
													   aUVs[pAttachment->m_aTriangles[i + 1]].x,
													   aUVs[pAttachment->m_aTriangles[i + 1]].y,
													   aUVs[pAttachment->m_aTriangles[i + 2]].x,
													   aUVs[pAttachment->m_aTriangles[i + 2]].y,
													   0.0f,
													   0.0f);
					...
					for(int l = 0; l < pAttachment->m_aEdges.size()-1; l++)
					{
						IGraphics::CLineItem LineItem(
								...
								aVerticesWorld[pAttachment->m_aEdges[l+1]].x,
					aVerticesWorld[pAttachment->m_aEdges[l+1]].y
							);
	...
	for(int i = 0; i < NumBones; i++)
	{
		...
		IGraphics::CLineItem LineItems[] = {
			IGraphics::CLineItem(Start.x, Start.y, Start.x + Direction.x*Offset + Normal.x, Start.y + Direction.y*Offset
	+ Normal.y), IGraphics::CLineItem(Start.x, Start.y, Start.x + Direction.x*Offset - Normal.x, Start.y +
	Direction.y*Offset - Normal.y), IGraphics::CLineItem(Start.x + Direction.x*Offset + Normal.x, Start.y +
	Direction.y*Offset + Normal.y, End.x, End.y), IGraphics::CLineItem(Start.x + Direction.x*Offset - Normal.x, Start.y
	+ Direction.y*Offset - Normal.y, End.x, End.y)
		};
...
void CRenderTools::RenderBuilding(
	vec2 Position, CAnimSkeletonInfo *pSkeleton, CTextureAtlas *pAtlas, int Team, int WeaponAngle)
{
	...
	pSkeleton->UpdateBones(Time, pAnimation, 0, WeaponAngle);
		...
		for(int i = 0; i < NumSlots; i++)
		{
			...
			switch(pAttachmentBase->m_Type)
			{
				case ATTACHMENT_SPRITE:
				{
					...
					if(pAttachment->m_SpecialType == AST_1_TEAM || pAttachment->m_SpecialType == AST_2_TEAM ||
					   pAttachment->m_SpecialType == AST_3_TEAM || pAttachment->m_SpecialType == AST_4_TEAM ||
...
void CRenderTools::RenderLine(vec2 p1, vec2 p2, float Size, vec4 Color)
{
	...
	IGraphics::CFreeformItem Freeform(
		p1.x - t.x, p1.y - t.y, p1.x + t.x, p1.y + t.y, p2.x - t.x, p2.y - t.y, p2.x + t.x, p2.y + t.y);
	Graphics()->QuadsDrawFreeform(&Freeform, 1);
