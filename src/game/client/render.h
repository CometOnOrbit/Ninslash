...
{
  public:
	CAnimAttachmentSprite()
	{
		m_Type = ATTACHMENT_SPRITE;
		m_SpecialType = AST_NONE;
	}
...
class CAnimSkeletonInfo
{
  ...
  public:
	...
	void UpdateBones(float Time = 0.0f,
...
class CRenderTools
{
  public:
	...
	void SelectSprite(
		struct CDataSprite *pSprite, int Flags = 0, int sx = 0, int sy = 0, float ox1 = 0.0f, float ox2 = 1.0f);
	...
	void RenderFreeHand(
		class CPlayerInfo *PlayerInfo, CTeeRenderInfo *pInfo, int Hand, vec2 Dir, vec2 Pos, bool Behind = false);
	...
	void RenderWeapon(const CWeaponSpec &Weapon,
					  vec2 Pos,
					  vec2 Dir,
					  float Size,
					  bool BeginQuads = false,
					  int Flags = 0,
					  float Alpha2 = 1.0f,
	...
	void RenderSkeleton(vec2 Position,
						const CTeeRenderInfo *pInfo,
						class CSkeletonAnimation *AnimData,
						float Rotation,
						CAnimSkeletonInfo *pSkeleton,
						CTextureAtlas *pAtlas,
						class CPlayerInfo *PlayerInfo = 0);
	void
	RenderBuilding(vec2 Position, CAnimSkeletonInfo *pSkeleton, CTextureAtlas *pAtlas, int Team, int WeaponAngle = 0);

	void RenderSkeleton(vec2 Pos,
						int Atlas,
						const char *Anim,
						float Time,
						vec2 Scale,
	...
	static void RenderEvalSkeletonAnim(TKeyframe *pKeyFrame,
									   int NumKeyframes,
									   float Time,
									   typename TKeyframe::KeyframeReturnType *pResult);
	...
	void SetShadersForWeapon(const CWeaponSpec &Weapon,
							 float Charge = 0.0f,
	...
	void RenderTilemap(CTile *pTiles,
					   int w,
					   int h,
					   float Scale,
					   vec4 Color,
