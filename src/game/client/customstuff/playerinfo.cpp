...
int CPlayerInfo::WeaponStaticType() const
{
	...
	return CWeaponCatalog::TryGetDefinition(m_Weapon.m_DefinitionId, &Definition) &&
				   Definition.m_Kind == EWeaponDefinitionKind::Static
			   ? Definition.m_StaticType
			   : -1;
}
...
void CPlayerInfo::Reset()
{
	...
	m_LastPhysicsVelocity = vec2(0, 0);
	m_HaveLastPhysicsVelocity = false;
...
{
	if(!m_HaveLastPhysicsVelocity)
	{
		m_LastPhysicsVelocity = PrevVel;
		m_HaveLastPhysicsVelocity = true;
	}
	...
	for(int i = 0; i < 2; i++)
	{
		...
		m_Hand[i].m_Vel *= 0.9f;
		m_Hand[i].m_Vel.x -= ((VelocityDelta.x / 2000) * (VelocityDelta.x / 2000)) / 2.0f;
		m_Hand[i].m_Vel.y += ((VelocityDelta.y / 2000) * (VelocityDelta.y / 2000)) / 2.0f;
	...
	if(m_Melee.m_FireTimer > 0)
	{
		...
		if((Animation()->m_Flip && m_Melee.m_TurnSpeed > 0.0f) || (!Animation()->m_Flip && m_Melee.m_TurnSpeed < 0.0f))
			m_Melee.m_Flip = true;
	...
	m_Weapon2RecoilVel -= VelocityDelta / 2000.0f;
...
void CPlayerInfo::Tick()
{
	...
	for(int i = 0; i < NUM_EFFECTS; i++)
	{
		if(m_EffectIntensity[i] > 0.0f)
		{
			...
			if(i != EFFECT_INVISIBILITY && i != EFFECT_DASH && i != EFFECT_FUEL)
