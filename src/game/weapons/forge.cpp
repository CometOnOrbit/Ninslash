...
{
	// TODO： This is a temporary hack to allow the forge to work with ranged weapons. It should be replaced with a
	// proper check for ranged weapon types.
	return Definition.m_Kind == EWeaponDefinitionKind::Modular && Definition.m_Part1 >= PART1_BASE1 &&
		   Definition.m_Part1 <= PART1_BASE6;
}

bool IsMelee(const CWeaponDefinition &Definition)
{
	return Definition.m_Kind == EWeaponDefinitionKind::Modular && Definition.m_Part1 >= PART1_MELEE &&
		   Definition.m_Part1 <= PART1_SPIN;
}
...
bool UpgradeProduct(const CWeaponSpec &Target,
					const CWeaponDefinition &Definition,
					bool Supercharge,
					CWeaponSpec *pProduct)
{
...
CForgeRecipe CForge::Resolve(const CWeaponSpec &Target,
							 ...
							 int MaterialAmmo)
{
	...
	if(!CWeaponCatalog::TryResolve(Target, &TargetProfile) || !CWeaponCatalog::TryResolve(Material, &MaterialProfile))
		return Recipe;
	CWeaponLuaForgeResult ModResult{};
	const int ModStatus =
		WeaponLuaResolveForge(Target, Material, TargetAmmo, MaterialAmmo, BaseCost, LevelCost, &ModResult);
	if(ModStatus < 0)
		return Recipe;
	if(ModStatus > 0)
	{
		CResolvedWeaponProfile ProductProfile;
		if(!CWeaponCatalog::TryResolve(ModResult.m_Product, &ProductProfile))
			return Recipe;
		Recipe.m_Operation = FORGEOP_MOD_RECIPE;
		Recipe.m_Product = ModResult.m_Product;
		Recipe.m_Cost = ModResult.m_Cost;
		...
		str_copy(Recipe.m_aRecipeName, ModResult.m_aName, sizeof(Recipe.m_aRecipeName));
		Recipe.m_Result = Recipe.m_Product == Target ? FORGERESULT_NO_CHANGE : FORGERESULT_SUCCESS;
		return Recipe;
	}

	if(TargetProfile.m_Definition.m_Custom || MaterialProfile.m_Definition.m_Custom)
		return Recipe;
	...
	if(MaterialDefinition.m_Kind == EWeaponDefinitionKind::Static && MaterialDefinition.m_StaticType == SW_UPGRADE)
	{
		...
		if((Material.m_Level != 0 && Material.m_Level != WEAPON_UPGRADE_SUPERCHARGE_LEVEL) ||
		   !UpgradeProduct(
			   Target, TargetDefinition, Material.m_Level == WEAPON_UPGRADE_SUPERCHARGE_LEVEL, &Recipe.m_Product))
			return Recipe;
	}
	else if(TargetDefinition.m_Kind == EWeaponDefinitionKind::Modular &&
			MaterialDefinition.m_Kind == EWeaponDefinitionKind::Modular && TargetDefinition.m_Part1 == PART1_MELEE &&
			MaterialDefinition.m_Part1 == PART1_MELEE && TargetDefinition.m_Part2 == MaterialDefinition.m_Part2)
	{
		Recipe.m_Operation = FORGEOP_SPIN;
		Recipe.m_Product =
			CWeaponCatalog::Modular(PART1_SPIN, TargetDefinition.m_Part2, (Target.m_Level + Material.m_Level) / 2);
	}
	else if(SameCategory(TargetDefinition, MaterialDefinition))
	{
		Recipe.m_Operation = FORGEOP_REPLACE_PART2;
		Recipe.m_Product = CWeaponCatalog::Modular(
			TargetDefinition.m_Part1, MaterialDefinition.m_Part2, (Target.m_Level + Material.m_Level) / 2);
	}
	else
		return Recipe;
...
bool CForge::Validate()
{
	...
	if(Resolve({}, RangedOdd, 0, 5, 2).m_Result != FORGERESULT_INVALID_RECIPE ||
	   ...
	   Resolve(Ranged, Ranged, 0, 5, 2).m_Result != FORGERESULT_NO_CHANGE || Cost(5, 2, 3, 4) != 19 ||
	   Cost(-20, 0, 0, 0) != 0 || Cost(900, 100, 1, 1) != FORGE_MAX_COST || MapAmmo(3, 7, 10) != 4 ||
	   MapAmmo(3, 0, 10) != 0 || MapAmmo(3, 7, 0) != 0)
		return false;
	// TODO
		...
		for(int TargetPart2 = PART2_BARREL1; TargetPart2 <= PART2_RAIL; ++TargetPart2)
			for(int MaterialPart1 = PART1_BASE1; MaterialPart1 <= PART1_BASE6; ++MaterialPart1)
				for(int MaterialPart2 = PART2_BARREL1; MaterialPart2 <= PART2_RAIL; ++MaterialPart2)
				{
					...
					const int ExpectedPart2Result =
						MaterialPart2 == TargetPart2 ? FORGERESULT_NO_CHANGE : FORGERESULT_SUCCESS;
					if(Part2.m_Result != ExpectedPart2Result || Part2.m_Operation != FORGEOP_REPLACE_PART2 ||
					   ...
					   Product.m_Part1 != TargetPart1 || Product.m_Part2 != MaterialPart2 ||
					   Part2.m_Product.m_Level != 3)
						return false;
				}
		...
		for(int TargetPart2 = PART2_MELEE1; TargetPart2 <= PART2_MELEE6; ++TargetPart2)
			for(int MaterialPart1 = PART1_MELEE; MaterialPart1 <= PART1_SPIN; ++MaterialPart1)
				for(int MaterialPart2 = PART2_MELEE1; MaterialPart2 <= PART2_MELEE6; ++MaterialPart2)
				{
					...
					CWeaponDefinition Product;
					const bool IsSpin =
						TargetPart1 == PART1_MELEE && MaterialPart1 == PART1_MELEE && TargetPart2 == MaterialPart2;
					if(Recipe.m_Result != FORGERESULT_SUCCESS ||
					   ...
					   Product.m_Part1 != (IsSpin ? PART1_SPIN : TargetPart1) || Product.m_Part2 != MaterialPart2 ||
					   Recipe.m_Product.m_Level != 2)
						return false;
				}
	...
	if(Spin.m_Result != FORGERESULT_SUCCESS || Spin.m_Operation != FORGEOP_SPIN || Spin.m_Product.m_Level != 3 ||
	   !CWeaponCatalog::TryGetDefinition(Spin.m_Product.m_DefinitionId, &SpinDefinition) ||
	   SpinDefinition.m_Part1 != PART1_SPIN || MeleePart2.m_Result != FORGERESULT_SUCCESS ||
	   MeleePart2.m_Operation != FORGEOP_REPLACE_PART2 ||
	...
	const CForgeRecipe HighBoundary =
		Resolve(CWeaponCatalog::Modular(PART1_BASE1, PART2_BARREL1, 8), SuperUpgrade, 0, 5, 2);
	...
	if(Overcharged.m_Result != FORGERESULT_SUCCESS || Overcharged.m_Operation != FORGEOP_UPGRADE ||
	   Overcharged.m_Product.m_Level != 1 || AtCap.m_Result != FORGERESULT_SUCCESS || AtCap.m_Product.m_Level != 6 ||
