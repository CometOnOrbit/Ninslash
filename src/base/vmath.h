...
template <typename T> class vector2_base
{
  public:
	union
	{
...
template <typename T> inline T length(const vector2_base<T> &a)
{
	return sqrtf(a.x * a.x + a.y * a.y);
}

template <typename T> inline T distance(const vector2_base<T> a, const vector2_base<T> &b)
{
	return length(a - b);
}

template <typename T> inline T dot(const vector2_base<T> a, const vector2_base<T> &b)
{
	return a.x * b.x + a.y * b.y;
}

template <typename T> inline vector2_base<T> normalize(const vector2_base<T> &v)
{
...
template <typename T>
inline vector2_base<T>
closest_point_on_line(vector2_base<T> line_point0, vector2_base<T> line_point1, vector2_base<T> target_point)
{
...
// ------------------------------------
template <typename T> class vector3_base
{
  public:
	union
	{
	...
	bool operator==(const vector3_base &v) const
	{
...
template <typename T> inline T length(const vector3_base<T> &a)
{
	return sqrtf(a.x * a.x + a.y * a.y + a.z * a.z);
}

template <typename T> inline T distance(const vector3_base<T> &a, const vector3_base<T> &b)
{
	return length(a - b);
}

template <typename T> inline T dot(const vector3_base<T> &a, const vector3_base<T> &b)
{
	return a.x * b.x + a.y * b.y + a.z * b.z;
}

template <typename T> inline vector3_base<T> normalize(const vector3_base<T> &v)
{
	T l = (T)(1.0f / sqrtf(v.x * v.x + v.y * v.y + v.z * v.z));
	return vector3_base<T>(v.x * l, v.y * l, v.z * l);
}

template <typename T> inline vector3_base<T> cross(const vector3_base<T> &a, const vector3_base<T> &b)
{
	return vector3_base<T>(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
}
...
template <typename T> class vector4_base
{
  public:
	union
	{
	...
	bool operator==(const vector4_base &v) const
	{
		return fabs(x - v.x) <= 1e-6f && fabs(y - v.y) <= 1e-6f && fabs(z - v.z) <= 1e-6f && fabs(w - v.w) <= 1e-6f;
	}
...
template <typename T> class matrix22_base
{
  public:
	T m00, m01;
	T m10, m11;
	...
	{
		return matrix22_base(m00 + m.m00, m01 + m.m01, m10 + m.m10, m11 + m.m11);
	}
	matrix22_base operator-(const matrix22_base &m) const
	{
		return matrix22_base(m00 - m.m00, m01 - m.m01, m10 - m.m10, m11 - m.m11);
	...
	vector2_base<T> operator*(const vector2_base<T> &v) const
	{
		return vector2_base<T>(m00 * v.x + m01 * v.y, m10 * v.x + m11 * v.y);
	}
	matrix22_base operator*(const matrix22_base &m) const
	{
		return matrix22_base(
			m00 * m.m00 + m01 * m.m10, m00 * m.m01 + m01 * m.m11, m10 * m.m00 + m11 * m.m10, m10 * m.m01 + m11 * m.m11);
	}
	...
	const matrix22_base &operator*=(const T &s)
	{
		m00 *= s;
		m01 *= s;
		m10 *= s;
...
// ------------------------------------
template <typename T> class matrix33_base
{
  public:
	T m00, m01, m02;
	T m10, m11, m12;
	...
	matrix33_base() {}
	matrix33_base(T n00, T n01, T n02, T n10, T n11, T n12, T n20, T n21, T n22)
	{
		m00 = n00;
		m01 = n01;
		m02 = n02;
	...
	static matrix33_base<T> identity() { return matrix33_base<T>(T(1), 0, 0, 0, T(1), 0, 0, 0, T(1)); }
	...
	{
		return matrix33_base<T>(m00 * m.m00 + m01 * m.m10 + m02 * m.m20,
