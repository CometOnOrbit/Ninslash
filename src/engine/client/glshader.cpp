	...
	if(!fileStream.is_open())
	{
		...
		if(fs_executable_path(aExecutable, sizeof(aExecutable)) == 0)
		{
			...
			if(pSlash)
			{
				*pSlash = 0;
				char aPath[1400];
				str_format(aPath, sizeof(aPath), "%s/%s", aExecutable, filePath);
				fileStream.clear();
...
GLuint LoadShader(const char *vertex_path, const char *fragment_path)
{
	...
	if(!result)
	{
		...
		glGetShaderInfoLog(vertShader, logLength, 0, infoLog);
	...
	if(!result)
	{
		...
		char *infoLog = (char *)malloc(sizeof(char) * logLength);
		glGetShaderInfoLog(fragShader, logLength, 0, infoLog);
		dbg_msg("gfx", "%s", infoLog);
		free(infoLog);
