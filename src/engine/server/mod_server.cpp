...
{
	CModCollection m_Collection;
	ILuaModRuntime *m_apRuntimes[CModCollection::MAX_MODS];
	int m_RuntimeCount;
	char m_aRoot[1024];
	...
	bool AddRecursive(const char *pID,char *pError,int ErrorSize)
	{
		...
		CModManifest Manifest;if(!ModPackageValidate(aPath,pID,m_aProtocol,&Manifest,pError,ErrorSize))return false;
...
bool CModServerRuntime::Load(const char *pRoot,const char *pIDs,const char *pProtocol,const char *pExpectedHash,char *pError,int ErrorSize)
{
	...
	{
		const CModCollection::CInstalledMod *pMod=m_pImpl->m_Collection.Get(aOrder[i]);if(!pMod)continue;
		bool HasScript=false;for(int f=0;f<pMod->m_Manifest.m_FileCount;f++)if(pMod->m_Manifest.m_aFiles[f].m_Type==MOD_FILE_SCRIPT)HasScript=true;
		...
		for(int f=0;f<pMod->m_Manifest.m_FileCount;f++)if(pMod->m_Manifest.m_aFiles[f].m_Type==MOD_FILE_SCRIPT)
		{
