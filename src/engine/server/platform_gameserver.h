...
inline bool PlatformAuthResultAllowsJoin(EPlatformAuthResult Result)
{
	return Result == PLATFORM_AUTH_OK;
}
inline bool PlatformAuthResultIsTerminalFailure(EPlatformAuthResult Result)
{
	return Result == PLATFORM_AUTH_INVALID_TICKET || Result == PLATFORM_AUTH_REPLAYED_TICKET ||
		   Result == PLATFORM_AUTH_UNAVAILABLE;
}
...
class IPlatformGameServer
{
  public:
	...
	virtual void UpdateMetadata(const char *pName,
								const char *pMap,
								int Players,
								int MaxPlayers,
