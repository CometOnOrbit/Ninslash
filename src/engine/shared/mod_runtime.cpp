...
class CLuaModRuntime : public ILuaModRuntime
{
	...
	static void *Allocator(void *pUser, void *pPointer, size_t OldSize, size_t NewSize)
	{
		...
		if(pResult)
			pSelf->m_MemoryUsed = (int)NewTotal;
		return pResult;
	}
	...
	static int LuaRandom(lua_State *pState)
	{
		...
		if(Args == 0)
		{
			lua_pushnumber(pState, X / 4294967296.0);
			return 1;
		}
		...
		{
			Low = High;
			High = luaL_checkinteger(pState, 2);
		}
		if(High < Low)
			return luaL_error(pState, "empty deterministic random interval");
	...
	void RegisterSandbox()
	{
		...
		const CLibrary aLibraries[] = {{LUA_GNAME, luaopen_base},
		...
		for(unsigned i = 0; i < sizeof(apDisabled) / sizeof(apDisabled[0]); i++)
		{
			...
			lua_setglobal(m_pState, apDisabled[i]);
		}
	...
	bool CopyError(char *pError, int ErrorSize)
	{
		...
		if(pError && ErrorSize > 0)
			str_copy(pError, pMessage ? pMessage : "Lua error", ErrorSize);
		if(m_pState)
			lua_pop(m_pState, 1);
		m_Active = false;
		return false;
  ...
  public:
	CLuaModRuntime() : m_pState(0), m_RandomState(1), m_MemoryUsed(0), m_InstructionCount(0), m_Active(false)
	{
		mem_zero(&m_Descriptor, sizeof(m_Descriptor));
	}
	...
	EModActivationResult Activate(const CModApiDescriptor &Descriptor)
	{
		...
		if(!m_pState)
			return MOD_ACTIVATION_RUNTIME_UNAVAILABLE;
	...
	{
		if(!m_Active || !m_pState || !pSource || SourceSize <= 0)
			return false;
		m_InstructionCount = 0;
		lua_sethook(m_pState, InstructionHook, LUA_MASKCOUNT, 1000);
		if(luaL_loadbufferx(m_pState, pSource, SourceSize, pName ? pName : "mod", "t") != LUA_OK ||
		   lua_pcall(m_pState, 0, 0, 0) != LUA_OK)
	...
	void OnModEvent(EModEvent Event, int ClientID, int Value)
	{
		...
		m_InstructionCount = 0;
		lua_sethook(m_pState, InstructionHook, LUA_MASKCOUNT, 1000);
		if(lua_pcall(m_pState, 3, 0, 0) != LUA_OK)
			CopyError(0, 0);
		lua_sethook(m_pState, 0, 0, 0);
	}
