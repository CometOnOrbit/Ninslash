...
class CContentCollection
{
  ...
  public:
	...
	const CInstalledContent *Get(int Index) const
	{
		return Index >= 0 && Index < m_ContentCount ? &m_aContent[Index] : 0;
	}
	bool AddManifest(const CContentManifest &Manifest, const char *pRoot, char *pError, int ErrorSize);
	bool AddValidatedPackage(
		const char *pRoot, const char *pPublishedFileID, const char *pProtocol, char *pError, int ErrorSize);
	bool Resolve(const char *const *ppRootIDs,
				 int RootCount,
				 int *pOrder,
				 int *pOrderCount,
