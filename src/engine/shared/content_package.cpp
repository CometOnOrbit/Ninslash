...
bool CopyFile(const char *pSource, const char *pDestination)
{
	...
	for(;;)
	{
		...
		if(Read == 0)
			break;
		if(io_write(Destination, aBuffer, Read) != Read)
		{
			Result = false;
			break;
		}
...
int ScanCallback(const char *pName, int IsDir, int DirType, void *pUser)
{
	...
	if(!pState->m_Valid || str_comp(pName, ".") == 0 || str_comp(pName, "..") == 0)
		return 0;
	char aRelative[512], aFull[1536];
	str_format(aRelative, sizeof(aRelative), "%s%s%s", pState->m_aRelative, pState->m_aRelative[0] ? "/" : "", pName);
	if(!ContentManifestIsSafeRelativePath(aRelative))
	{
		pState->m_Valid = Error(pState->m_pError, pState->m_ErrorSize, "unsafe file in content package");
		return 1;
	}
	str_format(aFull, sizeof(aFull), "%s/%s", pState->m_aRoot, aRelative);
	if(fs_is_symlink(aFull))
	{
		pState->m_Valid =
			Error(pState->m_pError, pState->m_ErrorSize, "symbolic links are forbidden in content packages");
		return 1;
	}
	if(IsDir)
	{
...
{
	if(!pRoot || !pRoot[0] || !aHash)
		return Error(pError, ErrorSize, "invalid content package root");
	int aOrder[CContentManifest::MAX_FILES];
	for(int i = 0; i < Manifest.m_FileCount; i++)
		aOrder[i] = i;
	for(int i = 0; i < Manifest.m_FileCount; i++)
		for(int j = i + 1; j < Manifest.m_FileCount; j++)
			if(str_comp(Manifest.m_aFiles[aOrder[i]].m_aPath, Manifest.m_aFiles[aOrder[j]].m_aPath) > 0)
			{
				int T = aOrder[i];
				aOrder[i] = aOrder[j];
				aOrder[j] = T;
	...
	char aNumber[32];
	str_format(aNumber, sizeof(aNumber), "%d", Manifest.m_SchemaVersion);
	Hash.Update(aNumber, str_length(aNumber) + 1);
	Hash.Update(ContentTypeName(Manifest.m_ContentType), str_length(ContentTypeName(Manifest.m_ContentType)) + 1);
	const char *apFields[] = {Manifest.m_aPublishedFileID,
							  Manifest.m_aName,
							  Manifest.m_aDescription,
							  Manifest.m_aAuthor,
							  Manifest.m_aVersion,
							  Manifest.m_aTargetProtocol,
							  Manifest.m_aContentRating};
	for(unsigned i = 0; i < sizeof(apFields) / sizeof(apFields[0]); i++)
		Hash.Update(apFields[i], str_length(apFields[i]) + 1);
	str_format(aNumber, sizeof(aNumber), "%d:%d", Manifest.m_Api.m_ApiVersion, Manifest.m_Api.m_Capabilities);
	Hash.Update(aNumber, str_length(aNumber) + 1);
	int aDependencyOrder[CContentManifest::MAX_DEPENDENCIES];
	for(int i = 0; i < Manifest.m_DependencyCount; i++)
		aDependencyOrder[i] = i;
	for(int i = 0; i < Manifest.m_DependencyCount; i++)
		for(int j = i + 1; j < Manifest.m_DependencyCount; j++)
			if(str_comp(Manifest.m_aDependencies[aDependencyOrder[i]].m_aPublishedFileID,
						Manifest.m_aDependencies[aDependencyOrder[j]].m_aPublishedFileID) > 0)
			{
				int T = aDependencyOrder[i];
				aDependencyOrder[i] = aDependencyOrder[j];
				aDependencyOrder[j] = T;
			}
	for(int i = 0; i < Manifest.m_DependencyCount; i++)
	{
	...
	for(int i = 0; i < Manifest.m_FileCount; i++)
	{
		...
		char aFull[1536];
		str_format(aFull, sizeof(aFull), "%s/%s", pRoot, pPath);
		IOHANDLE File = io_open(aFull, IOFLAG_READ);
		if(!File)
			return Error(pError, ErrorSize, "declared content file is missing");
		const long Size = io_length(File);
		if(Size < 0 || Size > 64 * 1024 * 1024 || TotalSize + (unsigned long long)Size > 256 * 1024 * 1024)
		{
			io_close(File);
			return Error(pError, ErrorSize, "content package size limit exceeded");
		}
		TotalSize += Size;
		Hash.Update(pPath, str_length(pPath) + 1);
		str_format(aNumber, sizeof(aNumber), "%d", Manifest.m_aFiles[aOrder[i]].m_Type);
		Hash.Update(aNumber, str_length(aNumber) + 1);
		unsigned char aSize[8];
		for(int Byte = 0; Byte < 8; Byte++)
			aSize[7 - Byte] = (unsigned char)((unsigned long long)Size >> (Byte * 8));
		...
		while(Remaining > 0)
		{
			...
			{
				io_close(File);
				return Error(pError, ErrorSize, "failed to read declared content file");
			}
			Hash.Update(aBuffer, Read);
			Remaining -= Read;
		}
		io_close(File);
	}
									 ...
									 int ErrorSize)
{
	...
	if(IDLength >= (int)sizeof(aID))
		return Error(pError, ErrorSize, "invalid PublishedFileID");
	mem_copy(aID, pID, IDLength);
	aID[IDLength] = 0;
	for(int i = 0; aID[i]; i++)
		if(aID[i] < '0' || aID[i] > '9')
			return Error(pError, ErrorSize, "invalid PublishedFileID");
	const char *pEntry = pSeparator + 1;
	if(!ContentManifestIsSafeRelativePath(pEntry))
		return Error(pError, ErrorSize, "unsafe Workshop map entry");
	CContentPackageIndex Index;
	if(!Index.Scan(pWorkshopRoot, pExpectedProtocol, pError, ErrorSize))
		return false;
	const CContentPackageIndex::CEntry *pPackage = Index.Find(aID);
	if(!pPackage || pPackage->m_Manifest.m_ContentType != CONTENT_TYPE_MAP)
		return Error(pError, ErrorSize, "Workshop locator is not a valid map package");
	const CContentManifest &Manifest = pPackage->m_Manifest;
	bool DeclaredMap = false;
	for(int i = 0; i < Manifest.m_FileCount; i++)
		if(Manifest.m_aFiles[i].m_Type == CONTENT_FILE_MAP && str_comp(Manifest.m_aFiles[i].m_aPath, pEntry) == 0)
			DeclaredMap = true;
	if(!DeclaredMap)
		return Error(pError, ErrorSize, "map entry is not declared by package");
	str_format(pPath, PathSize, "%s/%s", pPackage->m_aRoot, pEntry);
	return true;
}
...
{
	if(!pRoot || !pManifest)
		return Error(pError, ErrorSize, "invalid content package input");
	if(fs_is_symlink(pRoot))
		return Error(pError, ErrorSize, "symbolic links are forbidden in content packages");
	char aManifestPath[1536];
	str_format(aManifestPath, sizeof(aManifestPath), "%s/ninslash_content.json", pRoot);
	IOHANDLE File = io_open(aManifestPath, IOFLAG_READ);
	if(!File)
		return Error(pError, ErrorSize, "missing ninslash_content.json");
	const long Size = io_length(File);
	if(Size <= 0 || Size > 64 * 1024)
	{
		io_close(File);
		return Error(pError, ErrorSize, "invalid manifest size");
	}
	char *pData = static_cast<char *>(mem_alloc((unsigned)Size + 1, 1));
	const unsigned Read = io_read(File, pData, (unsigned)Size);
	io_close(File);
	pData[Read] = 0;
	const bool Parsed = Read == (unsigned)Size &&
						ContentManifestParse(pData, (int)Size, pExpectedProtocol, pManifest, pError, ErrorSize);
	mem_free(pData);
	if(!Parsed)
		return false;
	if(pExpectedPublishedFileID && pExpectedPublishedFileID[0] &&
	   str_comp(pManifest->m_aPublishedFileID, pExpectedPublishedFileID) != 0)
		return Error(pError, ErrorSize, "PublishedFileID does not match install directory");
	CScanState State;
	State.m_pManifest = pManifest;
	str_copy(State.m_aRoot, pRoot, sizeof(State.m_aRoot));
	State.m_aRelative[0] = 0;
	State.m_pError = pError;
	State.m_ErrorSize = ErrorSize;
	State.m_Valid = true;
	fs_listdir(pRoot, ScanCallback, 0, &State);
	if(!State.m_Valid)
		return false;
	char aActualHash[65];
	if(!ContentPackageComputeHash(pRoot, *pManifest, aActualHash, pError, ErrorSize))
		return false;
	if(str_comp_nocase(aActualHash, pManifest->m_aContentHash) != 0)
		return Error(pError, ErrorSize, "content package content hash mismatch");
	return true;
}
						 ...
						 int ErrorSize)
{
	...
	if(!pSourceRoot || !pWorkshopRoot || !pExpectedPublishedFileID || !pExpectedPublishedFileID[0] ||
	   !ContentPackageValidate(pSourceRoot, pExpectedPublishedFileID, pExpectedProtocol, &Manifest, pError, ErrorSize))
		return false;
	if(fs_makedir(pWorkshopRoot) != 0)
		return Error(pError, ErrorSize, "unable to create Workshop storage directory");
	...
	if(fs_makedir(aTemporary) != 0)
		return Error(pError, ErrorSize, "unable to create Workshop staging directory");
	...
	if(!Copied || !ContentPackageValidate(
					  aTemporary, pExpectedPublishedFileID, pExpectedProtocol, &StagedManifest, pError, ErrorSize))
	{
		RemoveTree(aTemporary);
	...
	if(HadTarget && fs_rename(aTarget, aBackup) != 0)
	{
		RemoveTree(aTemporary);
		return Error(pError, ErrorSize, "unable to replace installed Workshop package");
	}
	if(fs_rename(aTemporary, aTarget) != 0)
	{
		if(HadTarget)
			fs_rename(aBackup, aTarget);
		RemoveTree(aTemporary);
	...
	if(HadTarget)
		RemoveTree(aBackup);
	if(pManifest)
		*pManifest = StagedManifest;
	if(pStagedRoot && StagedRootSize > 0)
		str_copy(pStagedRoot, aTarget, StagedRootSize);
	return true;
}
